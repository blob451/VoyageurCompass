...
class TechnicalAnalysisEngineTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.sector = DataSector.objects.create(
            sectorKey="test_sector", sectorName="Test Sector", data_source="yahoo"
        )
    ...
    def test_relative_strength_with_sector_data(self):
        ...
        for i in range(504):  # 2 years of data
            current_date = self.base_date + timedelta(days=i)
            price_index = Decimal("1000") * (
                1 + Decimal(str(i * 0.0005))
            )  # Sector trend
        ...
        self.assertGreaterEqual(
            len(stock_prices), 504, "Insufficient stock price data for 2Y analysis"
        )
        ...
        sector_data = [
            ...
            for sp in DataSectorPrice.objects.filter(sector=self.sector).order_by(
                "date"
            )
        ]

        result = self.engine._calculate_relative_strength_1y(
            stock_prices, sector_data, []
        )
