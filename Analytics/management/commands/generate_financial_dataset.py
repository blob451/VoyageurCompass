...
class FinancialDatasetGenerator:
    ...
    def __init__(self):
        self.symbols = [
            ...
            'AMD', 'CRM', 'ORCL', 'ADBE', 'INTC', 'CSCO', 'IBM', 'PYPL',
            'JPM', 'BAC', 'WFC', 'GS', 'MS', 'C', 'USB', 'PNC',
            'JNJ', 'PFE', 'UNH', 'ABT', 'MRK', 'CVS', 'AMGN', 'GILD',
    ...
    def generate_instruction_sample(self) -> Dict[str, Any]:
        ...
        instruction_parts = [
            f"Generate a comprehensive investment analysis for {symbol} with score {score}/10"
        ]
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            type=int,
            default=10000,
            help='Number of instruction samples to generate (default: 10000)'
        )
        parser.add_argument(
            '--output-dir',
            type=str,
            default='Temp/',
            help='Output directory for dataset files (default: Temp/)'
        )
        parser.add_argument(
            '--split',
            action='store_true',
        ...
        try:
            ...
            if should_split:
                ...
                for split_name, split_data in splits.items():
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"Saved {len(split_data)} {split_name} samples to {filepath}"
                        )
                    )
            else:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Saved {len(dataset)} samples to {filepath}"
                    )
                )
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Dataset generation complete! Metadata saved to {metadata_file}"
                )
            )

        except Exception as e:
            self.stdout.write(
                self.style.ERROR(f"Error generating dataset: {str(e)}")
            )
            raise
