...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=float,
            default=100.0,
            help='Target prediction latency in milliseconds'
        )
        parser.add_argument(
            '--verbose',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS("=== Universal LSTM Performance Benchmarking ===")
        )
        ...
        if benchmark_type in ['speed', 'all']:
            ...
            results['speed'] = self.run_speed_benchmarks(
                options['sample_size'],
                options['iterations'],
                options['target_latency']
            )
        
        if benchmark_type in ['memory', 'all']:
            self.stdout.write("\n--- Running Memory Usage Benchmarks ---")
            results['memory'] = self.run_memory_benchmarks(
                options['sample_size']
            )
        ...
        if benchmark_type in ['accuracy', 'all']:
            ...
            results['accuracy'] = self.run_accuracy_benchmarks(
                options['sample_size']
            )
    ...
    def run_memory_benchmarks(self, sample_size: int) -> Dict[str, Any]:
        ...
        if hasattr(self.service, 'model') and self.service.model:
            ...
            model_memory_bytes = sum(
                ...
                for p in self.service.model.parameters()
            )
    ...
    def run_throughput_benchmarks(
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=concurrent_threads) as executor:
            future_to_stock = {
                executor.submit(make_prediction, stock): stock 
                for stock in test_stocks
            }
        ...
        throughput_results = {
            ...
            'throughput_metrics': {
                ...
                'avg_concurrent_latency_ms': np.mean([r['latency_ms'] for r in successful_concurrent]) if successful_concurrent else 0,
                'max_concurrent_latency_ms': np.max([r['latency_ms'] for r in successful_concurrent]) if successful_concurrent else 0
            },
    ...
    def run_accuracy_benchmarks(self, sample_size: int) -> Dict[str, Any]:
        ...
        if successful_predictions:
            ...
            realistic_predictions = [
                p for p in successful_predictions 
                if abs(p['price_change_pct']) < 50.0 and p['predicted_price'] > 0
            ]
            ...
            accuracy_results = {
                ...
                'prediction_stats': {
                    ...
                    'predictions_with_variation': len(set([round(p['predicted_price'], 2) for p in successful_predictions]))
                },
