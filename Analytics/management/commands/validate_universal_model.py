...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--hold-out-sector',
            type=str,
            help='Sector to hold out for cross-sector validation'
        )
        parser.add_argument(
            '--sample-size',
            type=int,
            default=20,
            help='Number of stocks to test per validation'
        )
        parser.add_argument(
            '--output-file',
            type=str,
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS("=== Universal LSTM Model Validation Framework ===")
        )
        ...
        try:
            ...
            self.stdout.write("Universal LSTM service initialized successfully")

        except Exception as e:
            raise CommandError(f"Failed to initialize validation components: {e}")
        ...
        if test_type in ['cross-sector', 'all']:
            ...
            results['cross_sector'] = self.run_cross_sector_validation(
                options.get('hold_out_sector'),
                options['sample_size']
            )

        if test_type in ['performance', 'all']:
            self.stdout.write("\n--- Running Performance Benchmarking ---")
            results['performance'] = self.run_performance_benchmarking(
                options['sample_size']
            )

        if test_type in ['zero-shot', 'all']:
            self.stdout.write("\n--- Running Zero-Shot Prediction Testing ---")
            results['zero_shot'] = self.run_zero_shot_testing(
                options['sample_size']
            )

        if test_type in ['transfer-learning', 'all']:
            self.stdout.write("\n--- Running Transfer Learning Evaluation ---")
            results['transfer_learning'] = self.run_transfer_learning_evaluation(
                options['sample_size']
            )
    ...
    def run_cross_sector_validation(
        ...
        results = {
            'test_type': 'cross_sector',
            'hold_out_sector': hold_out_sector,
            'sector_results': {},
            'summary': {}
        }
        ...
        for sector_name, sector_id in sectors.items():
            ...
            if self.verbose:
                ...
                self.stdout.write(
                    f"  {sector_name}: {success_rate:.1%} success, {avg_confidence:.3f} avg confidence"
                )
    ...
    def test_sector_performance(
        ...
        for stock_symbol in sector_stocks:
            ...
            try:
                # Make prediction
                result = self.service.predict_stock_price(
                    stock_symbol, 
                    use_cache=False
                )
    ...
    def run_performance_benchmarking(self, sample_size: int) -> Dict[str, Any]:
        ...
        return {
            ...
            'prediction_quality': {
                ...
                'avg_price_change_magnitude': np.mean([p['price_change_magnitude'] for p in prediction_quality]) if prediction_quality else 0,
        ...
        for source_sector in list(sectors.keys())[:3]:  # Test top 3 sectors
            ...
            for target_sector in list(sectors.keys())[:3]:
                ...
                if transfer_results:
                    ...
                    avg_confidence = np.mean([r['confidence'] for r in transfer_results if r['success']]) if any(r['success'] for r in transfer_results) else 0
    ...
    def get_stocks_by_sector(self, sector_name: str, limit: int) -> List[str]:
        ...
        try:
            ...
            try:
                stocks_from_db = Stock.objects.filter(
                    sector__sectorName__icontains=sector_name
            ...
            if len(diverse_stocks) < sample_size:
                try:
                    additional_stocks = Stock.objects.exclude(
                        symbol__in=diverse_stocks
                    ).values_list('symbol', flat=True)[:sample_size - len(diverse_stocks)]
