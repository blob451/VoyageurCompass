...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--symbol',
            type=str,
            help='Generate explanations for specific stock symbol only'
        )
        ...
        parser.add_argument(
            '--user',
            type=str,
            help='Generate explanations for specific user (username or email)'
        )
        ...
        parser.add_argument(
            '--since',
            type=str,
            help='Generate explanations for analyses since date (YYYY-MM-DD)'
        )

        parser.add_argument(
            '--until',
            type=str,
            help='Generate explanations for analyses until date (YYYY-MM-DD)'
        )
        ...
        parser.add_argument(
            ...
            type=int,
            default=10,
            help='Number of analyses to process at once (default: 10)'
        )
        ...
        parser.add_argument(
            '--force',
            action='store_true',
            help='Regenerate explanations even if they already exist'
        )
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS('Starting explanation generation process...')
        )
        ...
        if not status.get('llm_available', False):
            self.stdout.write(
                self.style.WARNING('LLM service not available - will use template fallback')
            )
        ...
        if options['user']:
            try:
                ...
                user = User.objects.get(
                    models.Q(username=options['user']) | models.Q(email=options['user'])
                )
        ...
        if options['dry_run']:
            ...
            for analysis in queryset[:10]:  # Show first 10
                self.stdout.write(
                    f"  - {analysis.stock.symbol} ({analysis.user.username}) - {analysis.as_of}"
                )
            if total_count > 10:
            ...
            for analysis in batch:
                try:
                    ...
                    if explanation_result:
                        ...
                        failed += 1
                        self.stdout.write(
                            self.style.ERROR(f"    ✗ Failed to generate explanation")
                        )
        ...
        if failed > 0:
            self.stdout.write(
                self.style.WARNING(
                    "Some explanations failed to generate. Check logs for details."
                )
            )
