"""
Fine-Tuning Management Views for Analytics app.
Provides endpoints for dataset generation, fine-tuning job management, and model evaluation.
"""
...
def generate_dataset(request):
    """
    Generate enhanced training dataset for fine-tuning.
    """
    try:
...
@extend_schema(
    ...
    description="List all fine-tuning jobs with their status"
)
@api_view(['GET'])
@permission_classes([IsAuthenticated])
...
@extend_schema(
    ...
    description="List all available fine-tuned models"
)
@api_view(['GET'])
@permission_classes([IsAuthenticated])
...
def export_dataset(request):
    ...
    try:
        ...
        return Response(
            {'error': f'Dataset export failed: {str(e)}'},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )


@extend_schema(
    summary="Get fine-tuning system status",
    description="Get overall status and capabilities of the fine-tuning system"
)
@api_view(['GET'])
@permission_classes([IsAuthenticated])
