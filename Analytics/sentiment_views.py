    ...
    responses={
        200: {
            ...
            "example": {
                ...
                "sources": {"Reuters": {"count": 10, "avg_score": 0.5}, "Bloomberg": {"count": 8, "avg_score": 0.3}},
                "articles": [],
            },
        }
    },
...
def stock_sentiment(request, symbol):
    ...
    try:
        ...
        if not refresh:
            cached_result = sentiment_analyzer.getCachedSentiment(cache_key, symbol, is_recent=(days <= 30))
            if cached_result:
        ...
        try:
            latest_analysis = AnalyticsResults.objects.filter(stock__symbol=symbol).latest("as_of")
        ...
        sentiment_analyzer.setCachedSentiment(cache_key, result, symbol, is_recent=(days <= 30))
