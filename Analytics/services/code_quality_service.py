...
class CodeQualityService:
    ...
    def analyze_project(self, include_patterns: List[str] = None) -> Dict[str, Any]:
        ...
        python_files = [
            f for f in python_files if "__pycache__" not in str(f) and "migrations" not in str(f) and f.is_file()
        ]
        ...
        analysis_results["summary"]["top_issues"] = sorted(issue_type_counts.items(), key=lambda x: x[1], reverse=True)[
            :10
        ]

        # Generate project-level recommendations
        analysis_results["summary"]["recommendations"] = self._generate_project_recommendations(analysis_results)

        logger.info(
            f"Project analysis complete: {analysis_results['files_analyzed']} files, "
    ...
    def _categorize_files_by_quality(self, file_results: List[Dict[str, Any]]) -> Dict[str, List[str]]:
        ...
        categories = {"excellent": [], "good": [], "fair": [], "poor": []}  # 90+  # 75-89  # 60-74  # <60
