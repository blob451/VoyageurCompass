"""
Enhanced Fine-Tuning Service providing comprehensive dataset generation and model management.
Implements fine-tuning lifecycle management with evaluation capabilities for financial models.
"""
...
from Analytics.services.financial_fine_tuner import (
    FINE_TUNING_AVAILABLE,
    FinancialDomainFineTuner,
)
...
class FineTuningManager:
    ...
    def generate_enhanced_dataset(
        self, num_samples: int = 5000, include_sentiment: bool = True, quality_threshold: float = 0.7
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            metadata = {
                ...
                "average_quality_score": (
                    sum(s["quality_score"] for s in training_samples) / len(training_samples) if training_samples else 0
                ),
                "detail_level_distribution": self._analyze_detail_distribution(training_samples),
                "sentiment_enhanced_count": sum(
                    1 for s in training_samples if s["metadata"].get("sentiment_enhanced", False)
                ),
    ...
    def start_fine_tuning_job(
        self, dataset_path: str, job_name: str = None, config_overrides: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            job_id = hashlib.blake2b(f"{job_name}_{time.time()}".encode(), digest_size=16).hexdigest()
            ...
            def run_training():
                try:
                    ...
                    results = fine_tuner.start_fine_tuning(
                        dataset_path=dataset_path, output_dir=str(self.model_dir / job_name)
                    )
                    ...
                    job_info.update({"status": "failed", "error": str(e), "failed_at": datetime.now().isoformat()})
