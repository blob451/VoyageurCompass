"""
Hybrid Analysis Coordinator for VoyageurCompass.
Coordinates sentiment analysis and LLM explanation generation for enhanced financial insights.
"""
...
class HybridAnalysisCoordinator:
    ...
    def generate_enhanced_explanation(self,
        ...
        try:
            ...
            if sentiment_data:
                ...
                logger.info(f"[HYBRID] Sentiment for {symbol}: {sentiment_label} (confidence: {sentiment_confidence:.2f})")
            else:
    ...
    def _has_clear_recommendation(self, content: str) -> bool:
        ...
        recommendation_keywords = ['BUY', 'SELL', 'HOLD', 'STRONG BUY', 'STRONG SELL', 
    ...
    def _generate_hybrid_cache_key(self,
                                 analysis_data: Dict[str, Any],
                                 detail_level: str,
        ...
        cache_hash = hashlib.md5(base_data.encode(), usedforsecurity=False).hexdigest()[:12]
        return f"enhanced_{symbol}_{cache_hash}"

    def _get_adaptive_ttl(self,
                         sentiment_data: Optional[Dict[str, Any]],
