...
class SentimentMetrics:
    ...
    def get_performance_trends(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'current_avg': float(np.mean(window_array[-10:])) if len(window_array) >= 10 else float(np.mean(window_array)),
...
class SentimentAnalyzer:
    ...
    def analyzeSentimentWithEnsemble(
        self,
        text: str,
        use_cache: bool = True
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check cache first
            if use_cache:
                cache_key = f"sentiment:ensemble:{hash(text)}"
                cached_result = cache.get(cache_key)
                if cached_result:
    ...
    def warm_cache(self, symbols: Optional[List[str]] = None):
        ...
        for symbol in symbols:
            try:
                ...
                cache.set(cache_key, {"warmed": True, "timestamp": datetime.now().isoformat()}, 
    ...
    def _extract_batch_features(self, texts: List[str]) -> np.ndarray:
        ...
        financial_terms = sum(len(re.findall(r'\b(?:USD|EUR|GBP|\$|%|Q[1-4]|CEO|CFO|IPO|M&A)\b', text, re.IGNORECASE)) for text in texts) / len(texts)
