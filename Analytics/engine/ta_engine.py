...
class TechnicalAnalysisEngine:
    ...
    def analyze_stock(
        ...
        try:
            ...
            sector_prices, industry_prices = self._get_sector_industry_data(
            ...
            indicators["rel1y"] = self._calculate_relative_strength_1y(
                stock_prices, sector_prices, industry_prices
            )
            indicators["rel2y"] = self._calculate_relative_strength_2y(
                stock_prices, sector_prices, industry_prices
            )
            ...
            for indicator_name, result in indicators.items():
                ...
                components[indicator_name] = {"raw": raw_value, "score": score}
    ...
    def _get_sector_industry_data(
        ...
        try:
            sector_key, industry_key = self.price_reader.get_stock_sector_industry_keys(
                symbol
            )
            start_date = (analysis_date - timedelta(days=3 * 365 + 30)).date()
            end_date = analysis_date.date()
            ...
            if sector_key:
                sector_prices = self.price_reader.get_sector_prices(
                    sector_key, start_date, end_date
                )

            if industry_key:
                industry_prices = self.price_reader.get_industry_prices(
                    industry_key, start_date, end_date
                )
    ...
    def _calculate_relative_strength_1y(
        ...
        try:
            ...
            # Stock 1Y return
            stock_current = float(
                stock_prices[-1].adjusted_close or stock_prices[-1].close
            )
            stock_1y_ago = float(
                stock_prices[-252].adjusted_close or stock_prices[-252].close
            )
    ...
    def _calculate_relative_strength_2y(
        ...
        try:
            ...
            stock_2y_ago = float(
                stock_prices[-baseline_index - 1].adjusted_close
                or stock_prices[-baseline_index - 1].close
            )
            ...
            if sector_prices and len(sector_prices) >= min_required_days:
                ...
                sector_current = float(sector_prices[-1].close_index)
                sector_2y_ago = float(
                    sector_prices[-sector_baseline_index - 1].close_index
                )
            ...
            if industry_prices and len(industry_prices) >= min_required_days:
                ...
                industry_current = float(industry_prices[-1].close_index)
                industry_2y_ago = float(
                    industry_prices[-industry_baseline_index - 1].close_index
                )
    ...
    def _calculate_candlestick_reversal(
        self, prices: List[PriceData]
    ) -> Optional[IndicatorResult]:
        """
    ...
    def _check_single_candle_patterns(self, price: PriceData) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                lower_shadow >= 1.5 * body_size
                and upper_shadow <= body_size * 0.7
                and body_size > total_range * 0.05
            ):
                return {"type": "bullish", "pattern": "hammer"}

            # Inverted Hammer (bullish) - long upper shadow at bottom of downtrend
            if (
                upper_shadow >= 1.5 * body_size
                and lower_shadow <= body_size * 0.7
                and body_size > total_range * 0.05
            ):
    ...
    def _calculate_support_resistance(
        ...
        try:
            ...
            # Check if near resistance (within 2%)
                ...
                nearest_resistance
                and abs(current_price - nearest_resistance) / nearest_resistance <= 0.02
            ):
            ...
            # Check if near support (within 2%)
                ...
                nearest_support
                and abs(current_price - nearest_support) / nearest_support <= 0.02
            ):
    ...
    def _find_resistance_levels(self, highs: List[float]) -> List[float]:
        ...
        for level in levels:
            if (
                not filtered_levels
                or abs(level - filtered_levels[-1]) / filtered_levels[-1] > 0.01
            ):
    ...
    def analyze_portfolio(
        self, portfolio_id: int, analysis_date: Optional[datetime] = None
    ) -> Dict:
        """
        ...
        return {
            ...
            "risk_metrics": {"beta": 1.2, "volatility": 0.18, "sharpe_ratio": 1.1},
    ...
    def calculate_technical_indicators(
        self, symbol: str, indicators: List[str] = None
    ) -> Dict:
        """
    ...
    def generate_recommendations(
        self, symbol: str, analysis_date: Optional[datetime] = None
    ) -> Dict:
        """
    ...
    def optimize_portfolio(
        self, portfolio_id: int, objective: str = "maximize_return"
    ) -> Dict:
        """
    ...
    def calculate_performance_metrics(
        self, portfolio_id: int, benchmark: str = "^GSPC"
    ) -> Dict:
        """
