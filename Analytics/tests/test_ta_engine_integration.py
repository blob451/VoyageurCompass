...
from Analytics.tests.fixtures import (
    AnalyticsTestDataFactory,
    OllamaTestService,
    TechnicalAnalysisTestEngine,
)
...
class TestLSTMPredictionInProduction(TestCase):
    ...
    def test_prediction_performance_impact(self):
        ...
        benchmark_result = PerformanceTestUtilities.benchmark_analysis_execution(
            self.ta_test_engine.calculate_prediction_score_with_data, "AAPL"
        )
        ...
        for symbol in symbols:
            benchmark = PerformanceTestUtilities.benchmark_analysis_execution(
                self.ta_test_engine.calculate_prediction_score_with_data, symbol
            )
