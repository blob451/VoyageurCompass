...
from Analytics.services.local_llm_service import (
    ConfidenceAdaptiveGeneration,
    SentimentEnhancedPromptBuilder,
...
class HybridIntegrationTestCase(TestCase):
    ...
    def test_sentiment_enhanced_prompt_builder(self):
        ...
        enhanced_prompt = prompt_builder.build_sentiment_aware_prompt(
            self.test_analysis_data, self.test_sentiment_data, "standard", "technical_analysis"
        )
        ...
        no_sentiment_prompt = prompt_builder.build_sentiment_aware_prompt(
            self.test_analysis_data, None, "standard", "technical_analysis"
        )
        ...
        for detail_level in ["summary", "standard", "detailed"]:
            prompt = prompt_builder.build_sentiment_aware_prompt(
                self.test_analysis_data, self.test_sentiment_data, detail_level
            )
    ...
    def test_confidence_adaptive_generation(self):
        ...
        high_confidence_options = adaptive_generator.get_confidence_weighted_options(
            self.test_sentiment_data, 0.8, "standard", "llama3.1:8b"  # High confidence sentiment  # High complexity
        )
        ...
        low_confidence_sentiment = {"sentimentScore": 0.1, "sentimentConfidence": 0.4, "sentimentLabel": "neutral"}

        low_confidence_options = adaptive_generator.get_confidence_weighted_options(
            low_confidence_sentiment, 0.3, "standard", "llama3.1:8b"  # Low complexity
        )
        ...
        options_70b = adaptive_generator.get_confidence_weighted_options(
            self.test_sentiment_data, 0.6, "detailed", "llama3.1:70b"
        )
    ...
    def test_hybrid_cache_functionality(self):
        ...
        test_result = {"content": "Test explanation content", "generation_time": 1.5, "model_used": "llama3.1:8b"}
    ...
    def test_sentiment_aware_explanation_generation(self):
        ...
        if not self.llm_service.is_available():
            ...
            result = self.llm_service.generate_sentiment_aware_explanation(
                analysis_data=self.test_analysis_data, sentiment_data=self.test_sentiment_data, detail_level="standard"
            )
    ...
    def test_hybrid_coordinator_integration(self):
        ...
        result = self.hybrid_coordinator.generate_enhanced_explanation(
            analysis_data=self.test_analysis_data, detail_level="standard"
        )
    ...
    def test_sentiment_aware_ttl_calculation(self):
        ...
        high_conf_ttl = self.llm_service._get_sentiment_aware_ttl(
            self.test_analysis_data, self.test_sentiment_data  # 0.82 confidence
        )

        # Low confidence sentiment should have shorter TTL
        low_confidence_sentiment = {"sentimentScore": 0.1, "sentimentConfidence": 0.3, "sentimentLabel": "neutral"}

        low_conf_ttl = self.llm_service._get_sentiment_aware_ttl(self.test_analysis_data, low_confidence_sentiment)
    ...
    def test_performance_metrics_tracking(self):
        ...
        with patch.object(self.sentiment_service, "analyzeNewsArticles") as mock_sentiment:
            with patch.object(self.llm_service, "generate_sentiment_aware_explanation") as mock_llm:
                ...
                result = self.hybrid_coordinator.generate_enhanced_explanation(self.test_analysis_data)
    ...
    def test_quality_assessment_functions(self):
        ...
        mentions_indicators = self.hybrid_coordinator._mentions_technical_indicators(
            content_with_indicators, self.test_analysis_data
        )
        ...
        alignment = self.hybrid_coordinator._check_sentiment_alignment(
            positive_content, self.test_sentiment_data  # Positive sentiment
        )
    ...
    def test_error_handling_and_fallbacks(self):
        ...
        with patch.object(
            self.sentiment_service, "analyzeNewsArticles", side_effect=Exception("Sentiment service error")
        ):
            with patch.object(self.llm_service, "generate_sentiment_aware_explanation") as mock_llm:
                mock_llm.return_value = {
        ...
        with patch.object(self.sentiment_service, "analyzeNewsArticles") as mock_sentiment:
            with patch.object(self.llm_service, "generate_sentiment_aware_explanation") as mock_llm:
                ...
                mock_llm.return_value = {
                    ...
                    "generation_time": 1.0,
                    "sentiment_enhanced": True,
                    "hybrid_coordination": {"quality_metrics": {"has_recommendation": True}},
                }

                # First request - should call services
                result1 = self.hybrid_coordinator.generate_enhanced_explanation(self.test_analysis_data, use_cache=True)

                # Second request - should use cache
                result2 = self.hybrid_coordinator.generate_enhanced_explanation(self.test_analysis_data, use_cache=True)
...
class SentimentLLMPerformanceTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.perf_test_data = {
            "symbol": "PERF_TEST",
            "score_0_10": 6.8,
            "weighted_scores": {"w_sma50vs200": 0.12, "w_rsi14": 0.09, "w_macd12269": 0.11},
            "news_articles": [{"title": "PERF_TEST Q2 Results", "summary": "Strong performance with 12% growth."}],
        }
    ...
    def test_response_time_with_sentiment_enhancement(self, mock_client_class):
        ...
        mock_client.generate.return_value = {"response": "Fast test response with BUY recommendation"}
    ...
    def test_cache_efficiency_with_hybrid_system(self):
        ...
        with patch.object(self.hybrid_coordinator.sentiment_service, "analyzeNewsArticles") as mock_sentiment:
            with patch.object(self.hybrid_coordinator.llm_service, "generate_sentiment_aware_explanation") as mock_llm:
                ...
                for _ in range(3):
                    result = self.hybrid_coordinator.generate_enhanced_explanation(self.perf_test_data, use_cache=True)
