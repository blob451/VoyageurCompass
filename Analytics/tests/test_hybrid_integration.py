...
class HybridIntegrationTestCase(TestCase):
    ...
    def test_sentiment_aware_explanation_generation(self, mock_client_class):
        """Test sentiment-aware explanation generation with mocked LLM."""
        # Setup mock
        mock_client = MagicMock()
        mock_client_class.return_value = mock_client
        
        with patch.object(self.llm_service, '_verify_model_availability', return_value=True):
            mock_client.generate.return_value = {
                'response': 'STRONG BUY recommendation for HYBRID_TEST. The technical score of 7.5/10 indicates bullish momentum with SMA crossover and strong RSI signals. Positive market sentiment (confidence: 82%) aligns with technical indicators, supporting the bullish thesis.'
            }
            
            # Test sentiment-aware generation
            ...
            self.assertEqual(sentiment_meta['sentiment_confidence'], 0.82)
            self.assertEqual(sentiment_meta['sentiment_label'], 'positive')
            
            # Validate content quality
            content = result['content']
            self.assertIn('BUY', content.upper())
            self.assertIn('HYBRID_TEST', content)
            self.assertIn('technical', content.lower())
    
    def test_hybrid_coordinator_integration(self):
        """Test full hybrid coordinator integration."""
        ...
        with patch.object(self.sentiment_service, 'analyzeNewsArticles') as mock_sentiment:
            ...
            with patch.object(self.llm_service, 'generate_sentiment_aware_explanation') as mock_llm:
                mock_llm.return_value = {
                    'content': 'HYBRID_TEST shows strong BUY signals with positive sentiment alignment.',
                    'generation_time': 1.2,
                    'model_used': 'llama3.1:8b',
                    ...
                    'sentiment_integration': {
                        ...
                        'generation_strategy': 'high_confidence_definitive'
                    }
                }
                ...
                self.assertIsNotNone(result)
                self.assertIn('content', result)
                self.assertIn('hybrid_coordination', result)
                
                # Validate coordination metadata
                coord_meta = result['hybrid_coordination']
                self.assertTrue(coord_meta['sentiment_integration_success'])
                self.assertTrue(coord_meta['post_processing_applied'])
                self.assertIn('quality_metrics', coord_meta)
                self.assertIn('overall_quality_score', coord_meta)
...
class SentimentLLMPerformanceTestCase(TestCase):
    ...
    def test_response_time_with_sentiment_enhancement(self, mock_client_class):
        ...
        with patch.object(llm_service, '_verify_model_availability', return_value=True):
            # Test sentiment-aware generation timing
            import time
            start_time = time.time()
            
            result = llm_service.generate_sentiment_aware_explanation(
                self.perf_test_data,
                {'sentimentScore': 0.5, 'sentimentConfidence': 0.7, 'sentimentLabel': 'positive'},
                'standard'
            )
            
            generation_time = time.time() - start_time
            
            # Should complete quickly (under 1 second for mocked generation)
            self.assertLess(generation_time, 1.0)
            
            if result:
                self.assertIn('content', result)
                self.assertTrue(result.get('sentiment_enhanced', False))
