...
class LLMPerformanceValidationTestCase(TestCase):
    ...
    def test_explanation_generation_performance_path(self, mock_client_class):
        """Test the performance path through explanation generation."""
        # Mock Ollama client
        mock_client = MagicMock()
        mock_client_class.return_value = mock_client
        
        # Mock successful model availability
        with patch.object(self.service, '_verify_model_availability', return_value=True):
            # Mock successful generation
            mock_client.generate.return_value = {
                'response': 'TEST1 receives a BUY recommendation based on strong technical indicators.'
            }
