...
from Analytics.tests.fixtures import AnalyticsTestDataFactory, OllamaTestService
...
class RealLocalLLMServiceTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.test_stock = Stock.objects.create(
            symbol="TEST", short_name="Test Corporation", long_name="Test Corporation Ltd.", exchange="NASDAQ"
        )
    ...
    def test_service_initialization_real(self):
        ...
        self.assertEqual(service.generation_timeout, 60)
    ...
    def test_real_explanation_generation_with_service_available(self):
        ...
        if service.is_available():
            ...
            result = service.generate_explanation(self.real_analysis_data, detail_level="standard")
    ...
    def test_real_explanation_generation_with_test_service(self):
        ...
        context_data = {"symbol": "TEST", "current_price": 150.00, "trend": "bullish"}
    ...
    def test_explanation_generation_different_detail_levels_real(self):
        ...
        for detail_level in detail_levels:
            if service.is_available():
                result = service.generate_explanation(self.real_analysis_data, detail_level=detail_level)
    ...
    def test_real_prompt_building(self):
        ...
        prompt = service._build_prompt(
            self.real_analysis_data, detail_level="standard", explanation_type="technical_analysis"
        )

        # Verify prompt contains key information
        self.assertIn("TEST", prompt)
        self.assertIn("7/10", prompt)
        self.assertIn("Key factors", prompt)
        self.assertIn("Analysis:", prompt)

        # Should contain key indicators
        self.assertIn("sma50vs200", prompt)
        self.assertIn("volsurge", prompt)
    ...
    def test_prompt_building_different_detail_levels(self):
        ...
        for detail_level in detail_levels:
            prompt = service._build_prompt(
                self.real_analysis_data, detail_level=detail_level, explanation_type="technical_analysis"
            )
            ...
            if detail_level == "summary":
                ...
                self.assertIn("Investment Thesis", prompt)
                self.assertIn("Key factors", prompt)
    ...
    def test_max_tokens_by_detail_level(self):
        ...
        self.assertEqual(tokens_summary, 90)
        self.assertEqual(tokens_standard, 180)
        self.assertEqual(tokens_detailed, 450)
    ...
    def test_cache_key_creation_consistency(self):
        ...
        cache_key1 = service._create_cache_key(self.real_analysis_data, "standard", "technical_analysis")

        cache_key2 = service._create_cache_key(self.real_analysis_data, "standard", "technical_analysis")

        # Same inputs should produce same cache key
        self.assertEqual(cache_key1, cache_key2)

        # Different detail level should produce different key
        cache_key3 = service._create_cache_key(self.real_analysis_data, "detailed", "technical_analysis")
    ...
    def test_real_batch_explanation_generation(self):
        ...
        results = service.generate_batch_explanations(analysis_batch, detail_level="summary")
    ...
    def test_real_service_status_reporting(self):
        ...
        # If service is available, check for model info
        if status.get("available"):
            ...
            # If not available, should have error info
            self.assertIn("error", status)
...
class RealLocalLLMServiceIntegrationTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.aapl_stock = Stock.objects.create(
            symbol="AAPL", short_name="Apple Inc.", long_name="Apple Inc. Corporation", exchange="NASDAQ"
        )
        ...
        self.realistic_analysis_data = AnalyticsTestDataFactory.create_comprehensive_analysis(self.aapl_stock)
    ...
    def test_realistic_explanation_generation_integration(self):
        ...
        if service.is_available():
            result = service.generate_explanation(self.analysis_dict, detail_level="detailed")
    ...
    def test_multiple_detail_levels_integration_real(self):
        ...
        for detail_level in ["summary", "standard", "detailed"]:
            if service.is_available():
                result = service.generate_explanation(self.analysis_dict, detail_level=detail_level)
        ...
        if len(results) >= 2:
            if "summary" in results and "standard" in results:
                self.assertLessEqual(
                    results["summary"]["word_count"], results["standard"]["word_count"] * 1.2  # Allow some variance
                )
