...
class FullWorkflowIntegrationTest(APITestCase):
    ...
    def test_complete_user_registration_and_portfolio_creation(self):
        ...
        portfolio_response = self.client.post(
            portfolio_url, portfolio_data, format="json"
        )
        ...
        for holding_data in holdings_data:
            ...
            holding_response = self.client.post(
                add_holding_url, holding_data, format="json"
            )
    ...
    def test_analytics_integration_workflow(self):
        ...
        portfolio_analysis_url = reverse(
            "analytics:portfolio-analysis", args=[portfolio.id]
        )
    ...
    def test_data_consistency_across_operations(self):
        ...
        # Update holding
        update_holding_url = reverse(
            "data:portfolio-update-holding", args=[portfolio_id]
        )
        ...
        # Remove holding
        remove_holding_url = reverse(
            "data:portfolio-remove-holding", args=[portfolio_id]
        )
        ...
        detail_response = self.client.get(portfolio_detail_url)
        active_holdings = [
            h for h in detail_response.data["holdings"] if h["is_active"]
        ]
...
class PerformanceIntegrationTest(APITestCase):
    ...
    def test_large_portfolio_performance(self):
        ...
        portfolio = Portfolio.objects.create(
            user=self.user, name="Large Portfolio", initial_value=Decimal("100000.00")
        )
