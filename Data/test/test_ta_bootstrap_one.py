...
class ClassificationUpsertTest(TestCase):
    ...
    def test_classification_upsert_idempotency(self):
        ...
        # First upsert
        created1, updated1, skipped1 = self.service.upsertClassification(
            classification_rows
        )

        # Second upsert with same data
        created2, updated2, skipped2 = self.service.upsertClassification(
            classification_rows
        )
...
class CompositeBuilderTest(TestCase):
    ...
    def setUp(self):
        ...
        self.sector = DataSector.objects.create(
            sectorKey="test_sector", sectorName="Test Sector"
        )
    ...
    def test_cap_weighted_calculation_accuracy(self):
        ...
        self.assertAlmostEqual(
            float(composite_data["close_index"]), float(expected_index), places=1
        )
