...
class DataSector(models.Model):
    ...
    sectorKey = models.CharField(
        max_length=50, unique=True, db_index=True, help_text="Normalized sector key (e.g., 'technology', 'healthcare')"
    )
    sectorName = models.CharField(max_length=200, help_text="Human-readable sector name")
    isActive = models.BooleanField(default=True, help_text="Whether this sector is actively tracked")
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    last_sync = models.DateTimeField(null=True, blank=True, help_text="Last time sector data was synced")
...
class DataIndustry(models.Model):
    ...
    industryKey = models.CharField(max_length=100, unique=True, db_index=True, help_text="Normalized industry key")
    industryName = models.CharField(max_length=200, help_text="Human-readable industry name")
    sector = models.ForeignKey(
        DataSector, on_delete=models.CASCADE, related_name="industries", help_text="Parent sector"
    )
    isActive = models.BooleanField(default=True, help_text="Whether this industry is actively tracked")
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    last_sync = models.DateTimeField(null=True, blank=True, help_text="Last time industry data was synced")
    data_source = models.CharField(
        max_length=20,
        choices=DataSourceChoices.choices,
        default=DataSourceChoices.YAHOO,
...
class DataSectorPrice(models.Model):
    ...
    sector = models.ForeignKey(DataSector, on_delete=models.CASCADE, related_name="prices", help_text="Related sector")
    ...
    fiftyTwoWeekChange = models.DecimalField(
        max_digits=10, decimal_places=6, null=True, blank=True, help_text="52-week change percentage"
    )
    fiftyDayAverage = models.DecimalField(
        max_digits=20, decimal_places=6, null=True, blank=True, help_text="50-day moving average"
    )
    twoHundredDayAverage = models.DecimalField(
        max_digits=20, decimal_places=6, null=True, blank=True, help_text="200-day moving average"
    )
    averageVolume = models.BigIntegerField(null=True, blank=True, help_text="Average daily volume")
    averageVolume3months = models.BigIntegerField(null=True, blank=True, help_text="3-month average daily volume")
...
class DataIndustryPrice(models.Model):
    ...
    industry = models.ForeignKey(
        DataIndustry, on_delete=models.CASCADE, related_name="prices", help_text="Related industry"
    )
    date = models.DateField(db_index=True, help_text="Trading date")
    close_index = models.DecimalField(
        max_digits=20,
...
class Stock(models.Model):
    ...
    short_name = models.CharField(max_length=100, blank=True, help_text="Short company name")
    long_name = models.CharField(max_length=255, blank=True, help_text="Full company name")

    # Trading information
    currency = models.CharField(max_length=10, default="USD", help_text="Trading currency")
    exchange = models.CharField(max_length=50, blank=True, help_text="Stock exchange (e.g., NASDAQ, NYSE)")
    ...
    country = models.CharField(max_length=50, blank=True, help_text="Country of headquarters")
    ...
    currentPrice = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True, help_text="Current trading price"
    )
    previousClose = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True, help_text="Previous day's closing price"
    )
    dayLow = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True, help_text="Day's low price")
    dayHigh = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True, help_text="Day's high price")
    regularMarketPrice = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True, help_text="Regular market price"
    )
    regularMarketOpen = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True, help_text="Regular market open price"
    )
    regularMarketDayLow = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True, help_text="Regular market day low"
    )
    regularMarketDayHigh = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True, help_text="Regular market day high"
    )
    regularMarketPreviousClose = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True, help_text="Regular market previous close"
    )
    fiftyTwoWeekLow = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True, help_text="52-week low price"
    )
    fiftyTwoWeekHigh = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True, help_text="52-week high price"
    )
    fiftyTwoWeekChange = models.DecimalField(
        max_digits=10, decimal_places=6, null=True, blank=True, help_text="52-week change percentage"
    )
    fiftyDayAverage = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True, help_text="50-day moving average"
    )
    twoHundredDayAverage = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True, help_text="200-day moving average"
    )
    beta = models.DecimalField(max_digits=6, decimal_places=4, null=True, blank=True, help_text="Beta coefficient")
    impliedVolatility = models.DecimalField(
        max_digits=6, decimal_places=4, null=True, blank=True, help_text="Implied volatility"
    )
    regularMarketVolume = models.BigIntegerField(null=True, blank=True, help_text="Regular market volume")
    averageVolume = models.BigIntegerField(null=True, blank=True, help_text="Average daily volume")
    averageVolume10days = models.BigIntegerField(null=True, blank=True, help_text="10-day average volume")
    averageVolume3months = models.BigIntegerField(null=True, blank=True, help_text="3-month average volume")
    ...
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    last_sync = models.DateTimeField(null=True, blank=True, help_text="Last time data was synced from Yahoo Finance")

    # Sector/Industry data tracking
    sectorUpdatedAt = models.DateTimeField(
        null=True, blank=True, help_text="Last time sector/industry data was updated"
    )
...
class StockPrice(models.Model):
    ...
    open = models.DecimalField(
        max_digits=10, decimal_places=2, validators=[MinValueValidator(Decimal("0"))], help_text="Opening price"
    )
    ...
    close = models.DecimalField(
        max_digits=10, decimal_places=2, validators=[MinValueValidator(Decimal("0"))], help_text="Closing price"
    )
...
class Portfolio(models.Model):
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="portfolios", help_text="Portfolio owner"
    )
    ...
    def update_value(self):
        ...
        total_value = sum(
            (holding.current_value for holding in self.holdings.filter(is_active=True)), start=Decimal("0")
        )
...
class PortfolioHolding(models.Model):
    ...
    portfolio = models.ForeignKey(
        Portfolio, on_delete=models.CASCADE, related_name="holdings", help_text="Parent portfolio"
    )
    stock = models.ForeignKey(
        Stock, on_delete=models.CASCADE, related_name="portfolio_holdings", help_text="Stock being held"
    )

    # Position details
    quantity = models.DecimalField(
        max_digits=12, decimal_places=4, validators=[MinValueValidator(Decimal("0"))], help_text="Number of shares"
    )
    ...
    unrealized_gain_loss = models.DecimalField(
        max_digits=12, decimal_places=2, default=0, help_text="Unrealized profit/loss"
    )
    unrealized_gain_loss_percent = models.DecimalField(
        max_digits=6, decimal_places=2, default=0, help_text="Unrealized profit/loss percentage"
    )

    # Metadata
    is_active = models.BooleanField(default=True, help_text="Whether this holding is active")
...
class AnalyticsResults(models.Model):
    ...
    stock = models.ForeignKey(
        Stock, on_delete=models.CASCADE, related_name="analytics_results", help_text="Related stock"
    )
    ...
    w_sma50vs200 = models.DecimalField(
        max_digits=10, decimal_places=6, null=True, blank=True, help_text="Weighted SMA 50/200 crossover score"
    )
    w_pricevs50 = models.DecimalField(
        max_digits=10, decimal_places=6, null=True, blank=True, help_text="Weighted price vs 50-day SMA score"
    )
    w_rsi14 = models.DecimalField(
        max_digits=10, decimal_places=6, null=True, blank=True, help_text="Weighted RSI(14) score"
    )
    w_macd12269 = models.DecimalField(
        max_digits=10, decimal_places=6, null=True, blank=True, help_text="Weighted MACD(12,26,9) histogram score"
    )
    w_bbpos20 = models.DecimalField(
        max_digits=10, decimal_places=6, null=True, blank=True, help_text="Weighted Bollinger %B (20,2) score"
    )
    w_bbwidth20 = models.DecimalField(
        max_digits=10, decimal_places=6, null=True, blank=True, help_text="Weighted Bollinger Bandwidth (20,2) score"
    )
    w_volsurge = models.DecimalField(
        max_digits=10, decimal_places=6, null=True, blank=True, help_text="Weighted Volume Surge score"
    )
    w_obv20 = models.DecimalField(
        max_digits=10, decimal_places=6, null=True, blank=True, help_text="Weighted OBV 20-day trend score"
    )
    w_rel1y = models.DecimalField(
        max_digits=10, decimal_places=6, null=True, blank=True, help_text="Weighted Relative Strength 1Y score"
    )
    w_rel2y = models.DecimalField(
        max_digits=10, decimal_places=6, null=True, blank=True, help_text="Weighted Relative Strength 2Y score"
    )
    w_candlerev = models.DecimalField(
        max_digits=10, decimal_places=6, null=True, blank=True, help_text="Weighted Candlestick Reversal score"
    )
    w_srcontext = models.DecimalField(
        max_digits=10, decimal_places=6, null=True, blank=True, help_text="Weighted Support/Resistance Context score"
    )

    # Sentiment analysis fields
    sentimentScore = models.FloatField(
        null=True, blank=True, help_text="Sentiment score (-1 to 1, where -1 is most negative, 1 is most positive)"
    )
    ...
    sentimentConfidence = models.FloatField(
        null=True, blank=True, help_text="Sentiment analysis confidence score (0 to 1)"
    )
    newsCount = models.IntegerField(
        null=True, blank=True, default=0, help_text="Number of news articles analyzed for sentiment"
    )
    lastNewsDate = models.DateTimeField(null=True, blank=True, help_text="Date of most recent news article analyzed")
    sentimentSources = models.JSONField(
        default=dict, blank=True, help_text="Breakdown of sentiment by source: {source: {count: n, avg_score: float}}"
    )

    # Composite results
    components = models.JSONField(
        default=dict, help_text="Raw and normalized values per indicator: {name: {raw: value, score: normalized_value}}"
    )
    composite_raw = models.DecimalField(
        max_digits=10, decimal_places=6, null=True, blank=True, help_text="Sum of all weighted scores"
    )
    score_0_10 = models.IntegerField(null=True, blank=True, help_text="Final composite score (0-10, rounded)")

    # LSTM Price Predictions
    prediction_1d = models.DecimalField(
        max_digits=10, decimal_places=4, null=True, blank=True, help_text="1-day price prediction from LSTM model"
    )
    prediction_7d = models.DecimalField(
        max_digits=10, decimal_places=4, null=True, blank=True, help_text="7-day price prediction from LSTM model"
    )
    prediction_30d = models.DecimalField(
        max_digits=10, decimal_places=4, null=True, blank=True, help_text="30-day price prediction from LSTM model"
    )
    prediction_confidence = models.FloatField(
        null=True, blank=True, help_text="Confidence score for LSTM predictions (0.0-1.0)"
    )
    model_version = models.CharField(
        max_length=20, null=True, blank=True, help_text="Version of LSTM model used for predictions"
    )
    prediction_timestamp = models.DateTimeField(
        null=True, blank=True, help_text="Timestamp when predictions were generated"
    )
    ...
    explanation_version = models.CharField(
        max_length=20, null=True, blank=True, help_text="Version of explanation system used"
    )
    narrative_text = models.TextField(null=True, blank=True, help_text="Natural language explanation text")
    narrative_language = models.CharField(max_length=5, default="en", help_text="Language code for narrative text")
    explained_at = models.DateTimeField(null=True, blank=True, help_text="Timestamp when explanation was generated")
    explanation_confidence = models.FloatField(
        null=True, blank=True, help_text="Confidence score for generated explanation (0.0-1.0)"
    )
...
class YahooFinanceCache(models.Model):
    """Yahoo Finance API response cache implementing graduated data persistence strategy."""
    ...
    symbol = models.CharField(max_length=20, db_index=True, help_text="Stock symbol (e.g., AAPL)")
    period = models.CharField(max_length=10, help_text="Data period (e.g., 1y, 3mo, max)")
    data_type = models.CharField(
        max_length=20, 
        choices=DATA_TYPE_CHOICES, 
        help_text="Type of Yahoo Finance data cached"
    )
    data = models.JSONField(help_text="Cached Yahoo Finance API response data")
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    expires_at = models.DateTimeField(help_text="When this cached data expires")
    fetch_success = models.BooleanField(default=True, help_text="Whether the API fetch was successful")
    error_message = models.TextField(
        null=True, blank=True, help_text="Error message if fetch failed"
    )
    ...
    def __str__(self):
        status = "✓" if self.fetch_success else "✗"
        return f"{status} {self.symbol} ({self.data_type}, {self.period}) - expires {self.expires_at}"

    def is_expired(self):
        """Validate cache entry expiration status against current timestamp."""        
        return timezone.now() > self.expires_at

    def refresh_expiry(self, hours=24):
        """Extend cache expiration timestamp with specified hour increment."""        
