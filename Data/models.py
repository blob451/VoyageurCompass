...
class DataSector(models.Model):
    ...
    sectorName = models.CharField(
        max_length=200, help_text="Human-readable sector name"
    )
    isActive = models.BooleanField(
        default=True, help_text="Whether this sector is actively tracked"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    last_sync = models.DateTimeField(
        null=True, blank=True, help_text="Last time sector data was synced"
    )
...
class DataIndustry(models.Model):
    ...
    industryKey = models.CharField(
        max_length=100, unique=True, db_index=True, help_text="Normalized industry key"
    )
    industryName = models.CharField(
        max_length=200, help_text="Human-readable industry name"
    )
    ...
    isActive = models.BooleanField(
        default=True, help_text="Whether this industry is actively tracked"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    last_sync = models.DateTimeField(
        null=True, blank=True, help_text="Last time industry data was synced"
    )
    data_source = models.CharField(
        max_length=20,
        choices=DataSourceChoices.choices,
...
class DataSectorPrice(models.Model):
    ...
    averageVolume = models.BigIntegerField(
        null=True, blank=True, help_text="Average daily volume"
    )
    averageVolume3months = models.BigIntegerField(
        null=True, blank=True, help_text="3-month average daily volume"
    )
    volume_agg = models.BigIntegerField(
        default=0, help_text="Aggregated volume for sector constituents"
    )
...
class DataIndustryPrice(models.Model):
    ...
    volume_agg = models.BigIntegerField(
        default=0, help_text="Aggregated volume for industry constituents"
    )
    constituents_count = models.IntegerField(
        help_text="Number of stocks included in composite"
    )
...
class Stock(models.Model):
    ...
    short_name = models.CharField(
        max_length=100, blank=True, help_text="Short company name"
    )
    long_name = models.CharField(
        max_length=255, blank=True, help_text="Full company name"
    )

    # Trading information
    currency = models.CharField(
        max_length=10, default="USD", help_text="Trading currency"
    )
    exchange = models.CharField(
        max_length=50, blank=True, help_text="Stock exchange (e.g., NASDAQ, NYSE)"
    )

    # Company details
    sector = models.CharField(max_length=100, blank=True, help_text="Business sector")
    industry = models.CharField(
        max_length=100, blank=True, help_text="Industry classification"
    )
    country = models.CharField(
        max_length=50, blank=True, help_text="Country of headquarters"
    )
    ...
    regularMarketVolume = models.BigIntegerField(
        null=True, blank=True, help_text="Regular market volume"
    )
    averageVolume = models.BigIntegerField(
        null=True, blank=True, help_text="Average daily volume"
    )
    averageVolume10days = models.BigIntegerField(
        null=True, blank=True, help_text="10-day average volume"
    )
    averageVolume3months = models.BigIntegerField(
        null=True, blank=True, help_text="3-month average volume"
    )
    shares_outstanding = models.BigIntegerField(
        default=0, help_text="Number of shares outstanding"
    )

    # Tracking fields
    is_active = models.BooleanField(
        default=True, help_text="Whether this stock is actively tracked"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    last_sync = models.DateTimeField(
        null=True, blank=True, help_text="Last time data was synced from Yahoo Finance"
    )
...
class Portfolio(models.Model):
    ...
    # Metadata
    is_active = models.BooleanField(
        default=True, help_text="Whether this portfolio is active"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def calculate_returns(self):
        ...
        if self.initial_value and self.initial_value != 0:
            return (
                (self.current_value - self.initial_value) / self.initial_value
            ) * Decimal("100")
        return Decimal("0")
...
class PortfolioHolding(models.Model):
    ...
    # Metadata
    is_active = models.BooleanField(
        default=True, help_text="Whether this holding is active"
    )
...
class AnalyticsResults(models.Model):
    ...
    score_0_10 = models.IntegerField(
        null=True, blank=True, help_text="Final composite score (0-10, rounded)"
    )
