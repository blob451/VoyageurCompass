...
class PriceReader:
    ...
    def get_stock_prices(self, symbol: str, start_date: date, end_date: Optional[date] = None) -> List[PriceData]:
        """
    ...
    def get_sector_prices(
        ...
        prices_qs = (
            sector.prices.filter(date__gte=start_date, date__lte=end_date).order_by("date").select_related("sector")
        )
    ...
    def get_industry_prices(
        ...
        prices_qs = (
            industry.prices.filter(date__gte=start_date, date__lte=end_date).order_by("date").select_related("industry")
        )
    ...
    def check_data_coverage(self, symbol: str, required_years: int = 2) -> Dict[str, Dict[str, Any]]:
        """
