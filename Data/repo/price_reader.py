...
class PriceReader:
    ...
    def get_stock_prices(
        ...
        prices_qs = (
            stock.prices.filter(date__gte=start_date, date__lte=end_date)
            .order_by("date")
    ...
    def get_sector_prices(
        self, sector_key: str, start_date: date, end_date: Optional[date] = None
        ...
        prices_qs = (
            sector.prices.filter(date__gte=start_date, date__lte=end_date)
            .order_by("date")
    ...
    def get_industry_prices(
        self, industry_key: str, start_date: date, end_date: Optional[date] = None
        ...
        prices_qs = (
            industry.prices.filter(date__gte=start_date, date__lte=end_date)
            .order_by("date")
    ...
    def get_stock_sector_industry_keys(
        self, symbol: str
        ...
        Raises:
            Stock.DoesNotExist: If stock symbol not found
        """
        stock = Stock.objects.select_related("sector_id", "industry_id").get(
            symbol=symbol.upper()
        )
    ...
    def check_data_coverage(
        self, symbol: str, required_years: int = 2
        ...
        start_date = end_date - timedelta(
            days=required_years * 365 + 30
        )  # Add buffer for leap years
        ...
        stock_prices = stock.prices.filter(date__gte=start_date).aggregate(
            earliest=Min("date"), latest=Max("date"), count=Count("id")
        )
        ...
        if stock_prices["count"] > 0:
            ...
            result["stock"]["gap_count"] = max(
                0, expected_trading_days - stock_prices["count"]
            )

        # Check sector data coverage
        if stock.sector_id:
            sector_prices = stock.sector_id.prices.filter(
                date__gte=start_date
            ).aggregate(earliest=Min("date"), latest=Max("date"), count=Count("id"))
            ...
            if sector_prices["count"] > 0:
                ...
                expected_trading_days = self._estimate_trading_days(
                    start_date, end_date
                )
                result["sector"]["gap_count"] = max(
                    0, expected_trading_days - sector_prices["count"]
                )

        # Check industry data coverage
        if stock.industry_id:
            industry_prices = stock.industry_id.prices.filter(
                date__gte=start_date
            ).aggregate(earliest=Min("date"), latest=Max("date"), count=Count("id"))
            ...
            if industry_prices["count"] > 0:
                ...
                result["industry"]["gap_count"] = max(
                    0, expected_trading_days - industry_prices["count"]
                )
