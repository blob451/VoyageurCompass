...
def market_overview(request):
    ...
    try:
        ...
        return Response({"error": "Failed to generate market overview"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def sync_watchlist(request):
    ...
    try:
        ...
        return Response({"error": "Failed to sync watchlist"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def sector_performance(request):
    ...
    try:
        ...
        sectors = {}
        stocks_iterator = Stock.objects.filter(is_active=True, sector__isnull=False).iterator(chunk_size=100)

        for stock in stocks_iterator:
            sector = stock.sector
            if sector not in sectors:
                sectors[sector] = {"name": sector, "stocks": [], "avg_change": 0, "total_market_cap": 0}
        ...
        for sector_data in sectors.values():
            if sector_data["stocks"]:
                ...
                sector_data["stocks"] = sorted(
                    sector_data["stocks"], key=lambda x: abs(x["change_percent"]), reverse=True
                )[:5]

        # Sort sectors by average change
        sorted_sectors = sorted(sectors.values(), key=lambda x: x["avg_change"], reverse=True)
        ...
        return Response({"error": "Failed to get sector performance"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        ...
        for symbol in symbols:
            ...
            try:
                ...
                if "error" not in stock_info:
                    ...
                    comparison[symbol] = {"symbol": symbol, "error": f"Stock data not found for {symbol}"}

        return Response(
            {"comparison": list(comparison.values()), "metrics": metrics, "timestamp": timezone.now().isoformat()}
        )

    except Exception as e:
        logger.error(f"Error comparing stocks: {str(e)}", exc_info=True)
        return Response({"error": "Failed to compare stocks"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def economic_calendar(request):
    ...
    # Economic calendar functionality currently unavailable
    # Returns empty data structure for API consistency
    ...
    return Response(
        {
            ...
            "days_requested": days,
            "status": "calendar_data_unavailable"
        }
    )
    ...
    try:
        ...
        for i in range(0, len(stock_symbols), batch_size):
            ...
            try:
                ...
                batch_results = yahoo_finance_service.get_multiple_stocks(batch_symbols, period="1d")
                ...
                logger.info(
                    f"Batch {i//batch_size + 1} completed: {batch_updated} updated, {batch_failed} failed. Progress: {progress_pct:.1f}%"
                )
        ...
        return Response({"error": "Failed to perform bulk update"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
