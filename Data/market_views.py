...
def market_overview(request):
    ...
    try:
        ...
        for symbol, name in indices.items():
            data = yahoo_finance_service.get_stock_data(
                symbol, period="1d", sync_db=False
            )
...
def sector_performance(request):
    ...
    try:
        ...
        stocks_iterator = Stock.objects.filter(
            is_active=True, sector__isnull=False
        ...
        sorted_sectors = sorted(
            sectors.values(), key=lambda x: x["avg_change"], reverse=True
        )
        ...
        for symbol in symbols:
            ...
            try:
                ...
                if "change_percent" in metrics and latest_price:
                    comparison[symbol]["change_percent"] = float(
                        latest_price.daily_change_percent
                    )
            ...
            try:
                ...
                for symbol, result in batch_results.items():
                    if result and "error" not in result:
                        ...
                        batch_failed += 1
                        logger.warning(
                            f"Failed to update {symbol}: {result.get('error', 'Unknown error')}"
                        )
                ...
                logger.error(
                    f"Batch processing failed for symbols {batch_symbols[:5]}...: {str(e)}"
                )
                total_failed += len(batch_symbols)

        logger.info(
            f"Bulk price update completed: {total_updated} updated, {total_failed} failed"
        )
