...
class StockViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def prices(self, request, pk=None):
        ...
        prices = StockPrice.objects.filter(stock=stock, date__gte=start_date, date__lte=end_date).order_by("-date")
    ...
    def sync(self, request, pk=None):
        ...
        result = yahoo_finance_service.get_stock_data(stock.symbol, period=period, sync_db=True)
        ...
        return Response({"message": "Stock data synchronized successfully", "stock": serializer.data})
    ...
    def historical(self, request, pk=None):
        ...
        historical_data = yahoo_finance_service.get_historical_data(stock.symbol, start_date, end_date)
    ...
    def trending(self, request):
        ...
        trending_symbols = getattr(
            settings, "TRENDING_STOCKS", ["AAPL", "MSFT", "GOOGL", "AMZN", "TSLA", "META", "NVDA", "JPM"]
        )

        # Filter active stocks and order by symbol for consistency
        stocks = Stock.objects.filter(symbol__in=trending_symbols, is_active=True).order_by("symbol")
    ...
    def realtime_quotes(self, request):
        ...
        if not isinstance(symbols, list):
            return Response({"error": "Symbols must be provided as a list"}, status=status.HTTP_400_BAD_REQUEST)

        # Limit number of symbols to prevent abuse
        if len(symbols) > 50:  # Reasonable limit
            return Response({"error": "Maximum 50 symbols allowed per request"}, status=status.HTTP_400_BAD_REQUEST)
...
class PortfolioViewSet(viewsets.ModelViewSet):
    ...
    def add_holding(self, request, pk=None):
        ...
        try:
            ...
            if yahoo_finance_service.validate_symbol(stock_symbol):
                sync_result = yahoo_finance_service.get_stock_data(stock_symbol, sync_db=True)
                if "error" not in sync_result:
                    stock = Stock.objects.get(symbol=stock_symbol)
                else:
                    return Response({"error": f"Stock {stock_symbol} not found"}, status=status.HTTP_404_NOT_FOUND)
            else:
                return Response({"error": f"Invalid stock symbol: {stock_symbol}"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def performance(self, request, pk=None):
        ...
        performance = {
            ...
            "total_gain_loss_percent": (float((total_gain_loss / total_cost) * 100) if total_cost > 0 else 0),
            "holdings": [],
        }
        ...
        for holding in holdings:
            ...
            if stock_data and "error" not in stock_data:
                ...
                if latest_price:
                    ...
                    updated_holdings.append({"symbol": symbol, "updated_price": float(holding.current_price)})
                else:
                    failed_updates.append({"symbol": symbol, "error": "No latest price data available after sync"})
            else:
                error_msg = stock_data.get("error", "Unknown error") if stock_data else "No data returned"
                failed_updates.append({"symbol": symbol, "error": error_msg})
    ...
    def remove_holding(self, request, pk=None):
        ...
        try:
            ...
            return Response({"error": f"Holding {symbol} not found in portfolio"}, status=status.HTTP_404_NOT_FOUND)
        ...
        try:
            ...
            if "quantity" in request.data:
                try:
                    ...
                    if quantity <= 0:
                        return Response(
                            {"error": "Quantity must be a positive number"}, status=status.HTTP_400_BAD_REQUEST
                        )
            ...
            if "average_price" in request.data:
                try:
                    ...
                    if average_price <= 0:
                        return Response(
                            {"error": "Average price must be a positive number"}, status=status.HTTP_400_BAD_REQUEST
                        )
                    holding.average_price = Decimal(str(average_price))
                except (ValueError, TypeError):
                    return Response(
                        {"error": "Average price must be a valid number"}, status=status.HTTP_400_BAD_REQUEST
                    )
            ...
            if "purchase_date" in request.data:
                try:
                    ...
                    if purchase_date > date.today():
                        return Response(
                            {"error": "Purchase date cannot be in the future"}, status=status.HTTP_400_BAD_REQUEST
                        )
    ...
    def allocation(self, request, pk=None):
        ...
        for sector_data in allocation["by_sector"].values():
            sector_data["percentage"] = float(
                (sector_data["value"] / total_value_float * 100) if total_value_float > 0 else 0
            )

        for industry_data in allocation["by_industry"].values():
            industry_data["percentage"] = float(
                (industry_data["value"] / total_value_float * 100) if total_value_float > 0 else 0
            )
