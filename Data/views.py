...
from Data.serializers import (
    MarketStatusSerializer, PortfolioDetailSerializer, PortfolioHoldingSerializer,
    PortfolioSerializer, StockDetailSerializer, StockPriceSerializer,
...
class StockViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def sync(self, request, pk=None):
        ...
        result = yahoo_finance_service.get_stock_data(
            stock.symbol, period=period, sync_db=True
        )
    ...
    def trending(self, request):
        ...
        stocks = Stock.objects.filter(
            symbol__in=trending_symbols, is_active=True
        ).order_by("symbol")
    ...
    def indices(self, request):
        ...
        include_full_data = (
            request.query_params.get("include_full_data", "false").lower() == "true"
        )
        ...
        for index in indices:
            try:
                price_data = yahoo_finance_service.get_stock_data(
                    index["symbol"], period="1d", sync_db=False
                )
                ...
                if "error" not in price_data:
                    ...
                    if (
                        prices_list
                        and len(prices_list) > 0
                        and prices_list[0] is not None
                    ):
                ...
                logger = logging.getLogger(__name__)
                logger.warning(
                    f"Failed to fetch data for index {index['symbol']}: {str(e)}"
                )
    ...
    def realtime_quotes(self, request):
        ...
        try:
            ...
            logger = logging.getLogger(__name__)
            logger.error(
                f"Real-time quotes retrieval failed for symbols {symbols}: {str(e)}"
            )
...
class PortfolioViewSet(viewsets.ModelViewSet):
    ...
    def update_prices(self, request, pk=None):
        ...
        stock_symbols = list(
            holdings.values_list("stock__symbol", flat=True).distinct()
        )

        # Make a single batch API call to fetch data for all symbols
        batch_stock_data = yahoo_finance_service.get_multiple_stocks(
            stock_symbols, period="1d"
        )
        ...
        for holding in holdings:
            ...
            if stock_data and "error" not in stock_data:
                ...
                error_msg = (
                    stock_data.get("error", "Unknown error")
                    if stock_data
        ...
        if holdings_to_update:
            PortfolioHolding.objects.bulk_update(
                holdings_to_update, ["current_price"], batch_size=100
            )
        ...
        if failed_updates:
            ...
            for failure in failed_updates:
                logger.warning(
                    f"Failed to update price for {failure['symbol']}: {failure['error']}"
                )
