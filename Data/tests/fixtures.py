...
from Data.models import (
    Stock, StockPrice, Portfolio, PortfolioHolding, 
    DataIndustry, DataSector, AnalyticsResults
)
...
class DataTestDataFactory:
    ...
    def create_test_stock(symbol="AAPL", company_name="Apple Inc.", sector="Technology"):
        ...
        data_sector, _ = DataSector.objects.get_or_create(
    ...
    def cleanup_test_data():
        ...
        PortfolioHolding.objects.filter(
            portfolio__name__icontains="test"
        ).delete()
        ...
        DataIndustry.objects.filter(
            industryKey__in=['software']
        ).delete()
        DataSector.objects.filter(
            sectorKey__in=['technology', 'financial_services']
        ).delete()
...
class YahooFinanceTestService:
        ...
        for company, symbols in symbol_database.items():
            if query_lower in company or query_lower.upper() in symbols:
                for symbol in symbols:
                    matches.append({
                        'symbol': symbol,
                        'name': self.get_stock_info(symbol)['shortName'],
                        'type': 'stock'
                    })
    ...
    def validate_symbol(self, symbol):
        ...
        valid_symbols = [
            ...
            'META', 'NVDA', 'JPM', 'BAC', 'WMT', 'V', 'JNJ',
            'PG', 'UNH', 'DIS', 'HD', 'MA', 'NFLX', 'CRM'
        ]
    ...
    def test_connection(self):
        ...
        return {
            ...
            'available_endpoints': [
                'get_stock_info', 'get_historical_data', 'get_multiple_quotes',
                'search_symbols', 'get_market_status', 'get_market_summary'
            ]
        }
