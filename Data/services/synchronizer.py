...
class DataSynchronizer:
    ...
    def _sync_price_history(self, stock: Stock, history: List[Dict]) -> int:
        ...
        for price_data in history:
            try:
                ...
                logger.warning(
                    f"Error syncing price for {stock.symbol} on {price_data.get('date')}: {str(e)}"
                )
                continue
    ...
    def sync_multiple_stocks(self, symbols: List[str], period: str = "1mo") -> Dict:
        ...
        for symbol in symbols:
            logger.info(
                f"Syncing {symbol} ({symbols.index(symbol) + 1}/{len(symbols)})"
            )
        ...
        logger.info(
            f"Batch sync completed: {total_success} successful, {total_failed} failed"
        )
        return summary
    ...
    def cleanup_old_prices(self, days_to_keep: int = 365) -> int:
        ...
        try:
            ...
            logger.info(
                f"Cleaned up {deleted_count} old price records older than {cutoff_date}"
            )
