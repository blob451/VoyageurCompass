...
from Analytics.ml.sector_mappings import TRAINING_STOCK_UNIVERSE, get_sector_mapper
from Data.models import DataSector, Stock
...
class SectorDataService:
    ...
    def ensure_training_stocks_in_database(self) -> Dict[str, Any]:
        ...
        for symbol in all_training_stocks:
            try:
                with transaction.atomic():
                    ...
                    logger.debug(
                        f"Stock {symbol} -> {sector_name} ({'created' if created else 'updated' if updated else 'exists'})"
                    )
        ...
        logger.info(
            f"Training stocks database sync complete: "
    ...
    def _ensure_sector_in_database(self, sector_name: str) -> DataSector:
        ...
        sector, created = DataSector.objects.get_or_create(
            sectorKey=sector_key, defaults={"sectorName": sector_name, "isActive": True, "data_source": "yahoo"}
        )
    ...
    def collect_sector_training_data(self, years_back: int = 3, min_data_points: int = 500) -> Dict[str, Any]:
        """
        ...
        for sector_name, stock_symbols in TRAINING_STOCK_UNIVERSE.items():
            ...
            sector_results = {"successful": 0, "failed": 0, "total_data_points": 0, "stocks": {}}
            ...
            logger.info(
                f"  {sector_name} complete: {sector_results['successful']}/{len(stock_symbols)} stocks, "
        ...
        logger.info(
            f"Data collection complete: {total_successful}/{total_successful + total_failed} stocks "
    ...
    def validate_sector_balance(self, min_stocks_per_sector: int = 5) -> Dict[str, Any]:
        ...
        for sector_name, count in sector_counts.items():
            if count < min_stocks_per_sector:
                ...
                validation_results["underrepresented_sectors"].append(
                    {"sector": sector_name, "current_count": count, "needed": min_stocks_per_sector - count}
                )
        ...
        if not validation_results["balanced"]:
            ...
            validation_results["recommendations"].append("Consider sector weighting during training to compensate")
        else:
            validation_results["recommendations"].append("Sector balance is adequate for universal training")
        ...
        logger.info(
            f"Sector balance validation complete: {'BALANCED' if validation_results['balanced'] else 'UNBALANCED'}"
        )

        return validation_results

    def sync_missing_stock_data(self, symbols: List[str], period: str = "2y") -> Dict[str, Any]:
        """
        ...
        sync_results = {"successful_syncs": [], "failed_syncs": {}, "total_symbols": len(symbols), "new_data_points": 0}
        ...
        logger.info(
            f"Data sync complete: {len(sync_results['successful_syncs'])}/{len(symbols)} stocks "
    ...
    def get_sector_statistics(self) -> Dict[str, Any]:
        ...
        training_stocks_in_db = Stock.objects.filter(symbol__in=training_stocks, is_active=True).count()
        ...
        statistics = {
            ...
            "avg_stocks_per_sector": (
                sum(sector_distribution.values()) / len(sector_distribution) if sector_distribution else 0
            ),
        ...
        logger.info(
            f"Sector statistics generated: {total_training_stocks} training stocks, "
    ...
    def prepare_training_data_summary(self) -> Dict[str, Any]:
        ...
        training_stocks = self.sector_mapper.get_all_training_stocks()
        data_quality = {"stocks_with_sufficient_data": 0, "stocks_with_insufficient_data": 0, "total_assessed": 0}
        ...
        if not balance_validation["balanced"]:
            summary["recommendations"].append(
                "Improve sector balance by adding more stocks to underrepresented sectors"
            )
