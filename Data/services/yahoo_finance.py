...
class YahooFinanceService:
    ...
    def validatePeriod(self, period: str) -> str:
        ...
        if period not in self.VALID_PERIODS:
            raise ValueError(
                f"Invalid period: {period}. Must be one of {sorted(self.VALID_PERIODS)}"
            )
        return period
    ...
    def validateInterval(self, interval: str) -> str:
        ...
        if interval not in self.VALID_INTERVALS:
            raise ValueError(
                f"Invalid interval: {interval}. Must be one of {sorted(self.VALID_INTERVALS)}"
            )
        return interval
    ...
    def getStockData(
        self, symbol: str, period: str = "1mo", syncDb: bool = True
    ) -> Dict:
        """camelCase wrapper for get_stock_data"""
        return self.get_stock_data(symbol, period, syncDb)

    def get_stock_data(
        self, symbol: str, period: str = "1mo", sync_db: bool = True
    ) -> Dict:
        """
        ...
        try:
            ...
            try:
                ...
                info = {
                    ...
                    "exchange": stock.exchange,
                    "lastSync": (
                        stock.last_sync.isoformat() if stock.last_sync else None
                    ),
                    "currentPrice": float(latest_price.close) if latest_price else None,
                    "previousClose": (
                        float(latest_price.close) if latest_price else None
                    ),
                    "from_cache": True,
                }
    ...
    def get_historical_data(
        ...
        try:
                        ...
                        "data": [
                            {
                                ...
                                "volume": p.volume,
                                "change_amount": (
                                    float(p.daily_change) if p.daily_change else None
                                ),
                                "change_percent": (
                                    float(p.daily_change_percent)
                                    if p.daily_change_percent
                                    else None
                                ),
                            }
                            for p in prices
            ...
            if sync_result["success"]:
                ...
                historical_data = {
                    "error": sync_result.get("error", "Failed to fetch data")
                }
    ...
    def get_multiple_stocks(
        ...
        for symbol in symbols:
            if (
                symbol in sync_results["results"]
                and sync_results["results"][symbol]["success"]
            ):
            ...
            try:
                ...
                if "429" in str(e) or "Too Many Requests" in str(e):
                    if attempt < self.maxRetries - 1:
                        ...
                        logger.warning(
                            f"Rate limited, attempt {attempt + 1}/{self.maxRetries}, waiting {delay:.1f}s"
                        )
                        time.sleep(delay)
                    else:
                        raise
    ...
    def fetchBatchHistorical(
        ...
        for i, ticker in enumerate(tickers):
            logger.info(
                f"Fetching {ticker} ({i+1}/{len(tickers)}) - {period} @ {interval}"
            )
            ...
            try:
                data = self._retryWithBackoff(
                    self._fetchSingleTicker, ticker, period, interval
                )
                results[ticker] = data
    ...
    def fetchSingleHistorical(
        self, ticker: str, period: str = "1wk", interval: str = "1d"
    ) -> List[Dict]:
    ...
    def _find_matching_ticker(
        self, columns: pd.MultiIndex, ticker: str
    ) -> Optional[str]:
        """
        ...
        for col_ticker in ticker_level:
            normalized_col = (
                col_ticker.upper().strip().replace("-", ".").replace("_", ".")
            )
        ...
        logger.warning(
            f"Could not find matching ticker for '{ticker}' in MultiIndex levels: {list(ticker_level)}"
        )
        return None
            ...
            for idx in row.index:
                if isinstance(idx, tuple) and len(idx) >= 2:
                    if (
                        ...
                        and column_type.lower() in idx[0].lower()
                    ):
                        return row[idx]

            logger.warning(
                f"Could not access {column_type} for ticker {ticker} in MultiIndex row"
            )
            return None

        except Exception as e:
            logger.warning(
                f"Error accessing MultiIndex column {column_type} for {ticker}: {str(e)}"
            )
            return None
            ...
            for date, row in history.iterrows():
                try:
                    ...
                    if isinstance(history.columns, pd.MultiIndex):
                        ...
                        matched_ticker = self._find_matching_ticker(
                            history.columns, ticker
                        )
                        ...
                        if matched_ticker:
                            ...
                            close_val = self._safe_multiindex_access(
                                row, "Close", ticker
                            )
                            ...
                            volume_val = self._safe_multiindex_access(
                                row, "Volume", ticker
                            )
                    else:
                        # Simple column names
                    ...
                    open_val = (
                        float(open_val) if pd.notna(open_val) else float(close_val)
                    )
                    high_val = (
                        float(high_val) if pd.notna(high_val) else float(close_val)
                    )
    ...
    def fetchSectorIndustryBatch(self, symbols: List[str]) -> Dict[str, Dict]:
        ...
        try:
            ...
            if not symbolsToFetch:
                logger.info(
                    "All symbols have recent sector/industry data, skipping fetch"
                )
                return {}

            logger.info(
                f"Fetching sector/industry for {len(symbolsToFetch)} symbols: {symbolsToFetch}"
            )
            ...
            for i, symbol in enumerate(symbolsToFetch):
                logger.info(
                    f"Fetching {symbol} sector/industry ({i+1}/{len(symbolsToFetch)})"
                )
                ...
                try:
                    data = self._retryWithBackoff(
                        self.fetchSectorIndustrySingle, symbol
                    )
                    if data and "error" not in data:
                        # Enhance with normalized keys for classification upsert
                        enhanced_data = {
                            "symbol": data["symbol"],
                            "sectorKey": self._normalizeSectorKey(
                                data.get("sector", "")
                            ),
                            "sectorName": data.get("sector", ""),
                            "industryKey": self._normalizeIndustryKey(
                                data.get("industry", "")
                            ),
                        ...
                        logger.info(
                            f"Successfully fetched sector/industry for {symbol}"
                        )
                    else:
                    ...
                    logger.error(
                        f"Failed to fetch sector/industry for {symbol}: {str(e)}"
                    )
    ...
    def fetchSectorIndustrySingle(self, symbol: str) -> Dict:
        ...
        try:
            ...
            logger.info(
                f"Retrieved sector/industry for {symbol}: {sector} / {industry}"
            )
            return result
    ...
    def upsertCompanyProfiles(self, profiles: Dict[str, Dict]) -> Tuple[int, int, int]:
        ...
        logger.info(
            f"Upsert complete: {created_count} created, {updated_count} updated, {skipped_count} skipped"
        )
        ...
        with transaction.atomic():
            for row in rows:
                try:
                    ...
                    if not industry_created and (
                        industry.industryName != industry_name
                        or industry.sector != sector
                    ):
                        industry.industryName = industry_name
                        industry.sector = sector
                        industry.last_sync = updated_at
                        industry.save(
                            update_fields=["industryName", "sector", "last_sync"]
                        )
                    ...
                    logger.error(
                        f"Error upserting classification for {row.get('symbol', 'unknown')}: {str(e)}"
                    )
                    skipped_count += 1
    ...
    def fetchStockEodHistory(
        self, symbol: str, startDate: datetime, endDate: datetime
    ) -> List[Dict]:
        """
        ...
        try:
            ...
            logger.info(
                f"Fetching EOD history for {symbol} from {startDate.date()} to {endDate.date()}"
            )
            ...
            if len(eod_data) > max_expected_trading_days * 1.05:
                ...
                logger.warning(
                    f"Trimming {len(eod_data)} records to {trim_to} expected trading days"
                )
                ...
                logger.warning(
                    f"Data count {len(eod_data)} is below minimum threshold of {min_threshold} trading days"
                )
    ...
    def composeSectorIndustryEod(
        ...
        try:
            logger.info(
                f"Computing sector/industry composites from {start_date.date()} to {end_date.date()}"
            )
            ...
            sectors = DataSector.objects.filter(
                isActive=True, stocks__isnull=False
            ).distinct()
            ...
            logger.info(
                f"Mapping stocks for {len(sectors)} sectors and {len(industries)} industries"
            )
            ...
            logger.info(
                f"Fetching unified price data for {len(all_stocks_set)} unique stocks"
            )
            all_prices = (
            ...
            logger.info(
                f"Indexed {len(all_prices)} price records across {len(price_dates)} dates"
            )
            ...
            total_entities = len(
                [s for s in sectors if s.id in sector_stock_mapping]
            ...
            logger.info(
                f"Using parallel processing with {max_workers} workers for {total_entities} entities"
            )
                ...
                if sector_tasks:
                    with ThreadPoolExecutor(max_workers=max_workers) as executor:
                        ...
                        for future in as_completed(future_to_entity):
                            ...
                            logger.debug(
                                f"Sector task completed: {entity_name} -> {created} records"
                            )
                ...
                if industry_tasks:
                    with ThreadPoolExecutor(max_workers=max_workers) as executor:
                        ...
                        for future in as_completed(future_to_entity):
                            ...
                            logger.debug(
                                f"Industry task completed: {entity_name} -> {created} records"
                            )
    ...
    def _createCompositesUnifiedBulk(
        ...
        try:
            ...
            for date in price_dates:
                ...
                for stock in stocks:
                    if (
                        stock in prices_by_stock_date
                        and date in prices_by_stock_date[stock]
                    ):
            ...
            model_class = (
                DataSectorPrice if entity_type == "sector" else DataIndustryPrice
            )
            ...
            for date in sorted(entity_prices_by_date.keys()):
                ...
                if date in existing_records:
                    ...
                    if (
                        record.constituents_count
                        != composite_data["constituents_count"]
                    ):
            ...
            if new_records:
                try:
                    model_class.objects.bulk_create(
                        new_records, batch_size=100, ignore_conflicts=True
                    )
                    created_count += len(new_records)
                    logger.debug(
                        f"Bulk created {len(new_records)} {entity_type} composite records"
                    )
                except Exception as e:
                    logger.warning(
                        f"Bulk create failed, falling back to individual creates: {str(e)}"
                    )
            ...
            if update_records:
                try:
                    ...
                    model_class.objects.bulk_update(
                        update_records, updated_fields, batch_size=100
                    )
                    created_count += len(update_records)
                    logger.debug(
                        f"Bulk updated {len(update_records)} {entity_type} composite records"
                    )
                except Exception as e:
                    logger.warning(
                        f"Bulk update failed, falling back to individual updates: {str(e)}"
                    )
            ...
            logger.error(
                f"Error creating {entity_type} composites for {entity}: {str(e)}"
            )
            return 0
    ...
    def _processEntityParallel(
        ...
        try:
            ...
            logger.debug(
                f"Parallel {entity_type} processing complete: {entity_name} -> {created} records"
            )
            return (entity_name, created)

        except Exception as e:
            logger.error(
                f"Error in parallel {entity_type} processing for {entity_name}: {str(e)}"
            )
    ...
    def _createSectorComposites(
        self, sector: "DataSector", start_date: datetime, end_date: datetime
    ) -> int:
        ...
        try:
            ...
            for date in price_dates:
                ...
                if composite_data:
                    ...
                    if created:
                        ...
                        sector_price.constituents_count
                        != composite_data["constituents_count"]
                    ):
            ...
            logger.error(
                f"Error creating sector composites for {sector.sectorName}: {str(e)}"
            )
            return 0

    def _createIndustryComposites(
        self, industry: "DataIndustry", start_date: datetime, end_date: datetime
    ) -> int:
        ...
        try:
            ...
            for date in price_dates:
                ...
                if composite_data:
                    ...
                    if created:
                        ...
                        industry_price.constituents_count
                        != composite_data["constituents_count"]
                    ):
                        # Update if constituent count changed
                        for key, value in composite_data.items():
            ...
            logger.error(
                f"Error creating industry composites for {industry.industryName}: {str(e)}"
            )
            return 0
    ...
    def fetchHistory(
        self, symbol: str, startDate: datetime, endDate: datetime, interval: str = "1d"
    ) -> List[Dict]:
        """
        ...
        try:
            ...
            logger.info(
                f"Fetching history for {symbol} from {startDate.date()} to {endDate.date()}"
            )
    ...
    def upsertStocksMetrics(self, row: Dict) -> bool:
        ...
        try:
            ...
            stock, created = Stock.objects.get_or_create(
                symbol=symbol, defaults=update_data
            )
    ...
    def ensure_postgresql_engine(self):
        ...
        if engine == "sqlite":
            ...
            logger.warning(
                f"Unexpected database engine '{engine}'. Expected 'postgresql'."
            )

        logger.info(f"Database engine verified: {engine}")

    def backfill_eod_gaps(
        self, symbol: str, required_years: int = 2, max_attempts: int = 3
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            calendar_days_needed = int(
                target_trading_days / 0.690420
            )  # Inverse of trading day ratio
            ...
            for attempt in range(1, max_attempts + 1):
                ...
                try:
                    ...
                    if (
                        not coverage["stock"]["has_data"]
                        or coverage["stock"]["gap_count"] > 50
                    ):
                        logger.info(f"Backfilling stock data for {symbol}")
                        stock_data = self.fetchStockEodHistory(
                    ...
                    if result["stock_backfilled"] > 0:
                        try:
                            ...
                            if (
                                ...
                                sector_key = self._normalizeSectorKey(
                                    stock_record.sector
                                )
                                industry_key = self._normalizeIndustryKey(
                                ...
                                logger.info(
                                    f"Creating sector/industry classification for {symbol}"
                                )
                                self.upsertClassification([classification_row])

                        except Exception as e:
                            logger.error(
                                f"Error creating classification for {symbol}: {str(e)}"
                            )

                    # Get sector/industry keys (should now exist after classification)
                    sector_key, industry_key = (
                        price_reader.get_stock_sector_industry_keys(symbol)
                    )
                    ...
                    if sector_key and not coverage["sector"]["has_data"]:
                        ...
                        sector_result = self.composeSectorIndustryEod(
                            (start_date, end_date)
                        )
                        result["sector_backfilled"] += sector_result.get(
                            "sector_prices_created", 0
                        )
                    ...
                    if industry_key and not coverage["industry"]["has_data"]:
                        logger.info(
                            f"Backfilling industry composite for {industry_key}"
                        )
                        industry_result = self.composeSectorIndustryEod(
                            (start_date, end_date)
                        )
                        result["industry_backfilled"] += industry_result.get(
                            "industry_prices_created", 0
                        )

                    # Check if gaps are adequately filled
                    final_coverage = price_reader.check_data_coverage(
                        symbol, required_years
                    )

                    stock_adequate = (
                        final_coverage["stock"]["has_data"]
                        and final_coverage["stock"]["gap_count"] <= 50
                    )

                    sector_adequate = not sector_key or (
                        final_coverage["sector"]["has_data"]
                        and final_coverage["sector"]["gap_count"] <= 50
                    )

                    industry_adequate = not industry_key or (
                        final_coverage["industry"]["has_data"]
                        and final_coverage["industry"]["gap_count"] <= 50
                    )

                    if stock_adequate and sector_adequate and industry_adequate:
                        result["success"] = True
                        logger.info(
                            f"Backfill complete for {symbol} after {attempt} attempts"
                        )
                        break
                    else:
                        logger.info(
                            f"Gaps still exist after attempt {attempt}, retrying..."
                        )
            ...
            with transaction.atomic():
                for row in stock_rows:
                    try:
                        ...
                        logger.warning(
                            f"Stock {row['symbol']} not found for price upsert"
                        )
                        continue
            ...
            with transaction.atomic():
                for row in rows:
                    ...
                    try:
                        ...
                        price, created = StockPrice.objects.get_or_create(
                            stock=stock, date=date, defaults=price_data
                        )
