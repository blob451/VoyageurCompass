...
class DataProvider:
    ...
    def _safe_fetch(self, ticker_symbol: str, retries: int = 0) -> Optional[yf.Ticker]:
        ...
        try:
            ...
            if "429" in error_str or "Too Many Requests" in error_str:
                logger.warning(
                    f"Rate limited on {ticker_symbol}, attempt {retries + 1}/{self.max_retries}"
                )
                if retries < self.max_retries - 1:
                    ...
                    logger.warning(
                        f"Max retries reached for {ticker_symbol}, will use mock data"
                    )
                    return None
            else:
    ...
    def fetch_stock_data(self, symbol: str, period: str = "1mo") -> Dict[str, Any]:
        ...
        try:
            ...
            if not ticker:
                ...
                logger.warning(
                    f"Unable to fetch data for {symbol} due to API limitations"
                )
            ...
            if history is not None and not history.empty:
                ...
                logger.info(
                    f"Successfully fetched data for {symbol} - {len(stock_data['history'])} days of history"
                )
            else:
    ...
    def validate_symbol(self, symbol: str) -> bool:
        ...
        try:
            ...
            logger.warning(
                f"Cannot validate {symbol} due to rate limiting, assuming valid"
            )
            return True
