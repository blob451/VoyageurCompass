...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.checkIndexes()
            self.stdout.write(
                self.style.SUCCESS("[OK] Database verification completed successfully")
            )
        except Exception as e:
            self.stdout.write(
                self.style.ERROR(f"[ERROR] Verification failed: {str(e)}")
            )
            raise
    ...
    def checkDataSeparation(self):
        ...
        yahooPrice = StockPrice.objects.filter(
            data_source=DataSourceChoices.YAHOO
        ).count()
        mockPrice = StockPrice.objects.filter(
            data_source=DataSourceChoices.MOCK
        ).count()
        ...
        with connection.cursor() as cursor:
            for model, expected_columns, description in expected_indexes:
                ...
                try:
                    ...
                    table_indexes = connection.introspection.get_indexes(
                        cursor, table_name
                    )

                    # Check if expected columns are indexed
                    index_found = self._check_columns_indexed(
                        table_indexes, expected_columns
                    )

                    if index_found:
                        self.stdout.write(
                            f"  [OK] {description}: columns {expected_columns} are indexed"
                        )
                    else:
                        self.stdout.write(
                            f"  [WARN] Missing {description}: columns {expected_columns} not indexed"
                        )

                except Exception as e:
                    self.stdout.write(
                        f"  [WARN] Could not verify indexes for {table_name}: {str(e)}"
                    )
