...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        markdown_content.append(
            "This document contains the first 5 and last 5 rows of each DATA table containing AAPL-related information with all required fields for the VoyageurCompass financial analysis system."
        )
        markdown_content.append("")
        ...
        for model in data_models:
            ...
            field_names = {
                f.name for f in model._meta.get_fields() if isinstance(f, djm.Field)
            }

            # Check if this model has any of the strict fields or is related to AAPL
            if field_names & all_strict_fields or model.__name__ in [
                "Stock",
                "StockPrice",
            ]:
                try:
        ...
        markdown_content.append("")
        markdown_content.append(
            "This data export shows the successful implementation of:"
        )
        markdown_content.append(
            "- All required Stocks category fields in the Stock model"
        )
        markdown_content.append(
            "- All required Industry & Sector fields in DataSectorPrice and DataIndustryPrice models"
        )
        markdown_content.append(
            "- Historical price data with adjusted_close and volume in StockPrice model"
        )
        markdown_content.append(
            "- 3 years of AAPL data successfully imported from Yahoo Finance"
        )
        markdown_content.append("")
        ...
        self.stdout.write(
            f"✓ Document contains data from {len([m for m in data_models if any(f.name in all_strict_fields for f in m._meta.get_fields())])} DATA tables"
        )
