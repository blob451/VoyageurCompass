...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not options.get("force") and not self.check_recency_policy():
            self.stdout.write(
                self.style.SUCCESS(
                    "Recent data exists, skipping fetch (use --force to override)"
                )
            )
            return
    ...
    def validate_db_engine(self) -> bool:
        ...
        try:
            ...
            if "sqlite" in db_engine.lower():
                self.stdout.write(
                    self.style.ERROR(
                        "ABORT: SQLite database detected. This command requires PostgreSQL."
                    )
                )
                return False
    ...
    def verify_schema(self) -> bool:
        ...
        try:
            ...
            missing_stock_fields = [
                f for f in required_stock_fields if f not in stock_fields
            ]
            if missing_stock_fields:
                self.stdout.write(
                    self.style.ERROR(
                        f"ABORT: Missing Stock fields: {missing_stock_fields}"
                    )
                )
                return False
            ...
            missing_sector_fields = [
                f for f in required_sector_fields if f not in sector_fields
            ]
            if missing_sector_fields:
                self.stdout.write(
                    self.style.ERROR(
                        f"ABORT: Missing DataSectorPrice fields: {missing_sector_fields}"
                    )
                )
                return False
            ...
            missing_industry_fields = [
                f for f in required_sector_fields if f not in industry_fields
            ]
            if missing_industry_fields:
                self.stdout.write(
                    self.style.ERROR(
                        f"ABORT: Missing DataIndustryPrice fields: {missing_industry_fields}"
                    )
                )
                return False

            self.stdout.write(
                "✓ Schema verification passed - all required fields present"
            )
            return True
        ...
        try:
            ...
            try:
                ...
                if stock.sectorUpdatedAt and stock.sectorUpdatedAt >= threshold_date:
                    self.stdout.write(
                        "Recent sector/industry data exists (< 3 years old)"
                    )
                    return False
    ...
    def pull_aapl_data(self):
        ...
        try:
            ...
            if "error" not in sector_data:
                ...
                self.stdout.write(
                    f"✓ Sector/industry data: {'success' if success else 'failed'}"
                )

            # 3. Fetch 3-year history
            history_data = yahoo_finance_service.fetchHistory(
