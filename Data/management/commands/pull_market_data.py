...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            action="store_false",
            dest="batch",
            help="Disable batch mode and process tickers individually",
        )
    ...
    def handle(self, *args, **options):
        ...
        if not tickers:
            raise CommandError(
                "No valid tickers provided. Please specify at least one ticker symbol."
            )
        ...
        try:
            if batch_mode and len(tickers) > 1:
                ...
                for ticker, bars in results.items():
                    if bars:
                        ...
                        self.stdout.write(
                            self.style.WARNING(f"{ticker}: No data fetched")
                        )
            ...
            ticker_list = (
                ", ".join(tickers)
                if len(tickers) <= 5
                else f"{', '.join(tickers[:5])}, and {len(tickers)-5} more"
            )
            raise CommandError(
