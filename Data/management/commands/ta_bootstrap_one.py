...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--symbol", type=str, required=True, help="Stock ticker symbol (e.g., AAPL)"
        )
        parser.add_argument(
            "--start",
            type=str,
    ...
    def fetchAndUpsertClassification(self, service, symbol: str) -> str:
        ...
        try:
            ...
            if "error" in classification_data:
                raise CommandError(
                    f"Failed to fetch classification for {symbol}: {classification_data['error']}"
                )
            ...
            created, updated, skipped = service.upsertClassification(
                classification_rows
            )
    ...
    def fetchAndPopulateEodData(
        self, service, symbol: str, start_date: datetime, end_date: datetime
    ) -> int:
        ...
        try:
            ...
            try:
                ...
                raise CommandError(
                    f"Stock record for {symbol} not found. Classification step may have failed."
                )
            ...
            self.stdout.write(
                f"  EOD data: {records_created} created, {records_updated} updated"
            )
    ...
    def buildSectorIndustryComposites(
        self, service, symbol: str, start_date: datetime, end_date: datetime
    ) -> dict:
        ...
        try:
            ...
            if not stock.sector_id or not stock.industry_id:
                raise CommandError(
                    f"Stock {symbol} missing sector/industry classification"
                )
            ...
            actual_start = datetime.combine(
                price_range["min_date"], datetime.min.time()
            )
            ...
            composite_result = service.composeSectorIndustryEod(
    ...
    def verifyDataIntegrity(
        ...
        try:
            ...
            sector_count = DataSectorPrice.objects.filter(
                sector=stock.sector_id
            ).count()
            industry_count = DataIndustryPrice.objects.filter(
                industry=stock.industry_id
            ).count()

            # Get composite date range for verification
            sector_range = DataSectorPrice.objects.filter(
                sector=stock.sector_id
            ).aggregate(min_date=models.Min("date"), max_date=models.Max("date"))

            result = (
                f"Stock: {total_price_count} total records ({price_range['min_date']} to {price_range['max_date']}), "
            ...
            if (
                sector_range["min_date"] == price_range["min_date"]
                and sector_range["max_date"] == price_range["max_date"]
                and sector_count == total_price_count
            ):
                self.stdout.write(
                    "  ✓ Composite data perfectly aligned with stock price data"
                )
            else:
                self.stdout.write(
                    "  ⚠ Composite data range differs from stock price range"
                )
    ...
    def printVerificationOutput(self, symbol: str):
        ...
        try:
            ...
            for s in stocks_first:
                self.stdout.write(
                    f"  {s.symbol} | {s.short_name} | {s.sector_id_id if s.sector_id else 'None'} | {s.industry_id_id if s.industry_id else 'None'}"
                )
            ...
            for p in prices_first:
                self.stdout.write(
                    f"  {p.date} | {p.open} | {p.high} | {p.low} | {p.close} | {p.adjusted_close} | {p.volume}"
                )
            ...
            for s in sectors_first:
                self.stdout.write(
                    f"  {s.id} | {s.sectorKey} | {s.sectorName} | {s.isActive}"
                )
            ...
            for i in industries_first:
                self.stdout.write(
                    f"  {i.id} | {i.industryKey} | {i.industryName} | {i.sector.sectorName} | {i.isActive}"
                )
            ...
            if stock.sector_id:
                ...
                sector_prices_first = DataSectorPrice.objects.filter(
                    sector=stock.sector_id
                ).order_by("date")[:5]
                sector_prices_last = DataSectorPrice.objects.filter(
                    sector=stock.sector_id
                ).order_by("-date")[:5]

                for sp in sector_prices_first:
                    self.stdout.write(
                        f"  {sp.date} | {sp.close_index} | {sp.volume_agg} | {sp.constituents_count} | {sp.method}"
                    )
            ...
            if stock.industry_id:
                ...
                industry_prices_first = DataIndustryPrice.objects.filter(
                    industry=stock.industry_id
                ).order_by("date")[:5]
                industry_prices_last = DataIndustryPrice.objects.filter(
                    industry=stock.industry_id
                ).order_by("-date")[:5]

                for ip in industry_prices_first:
                    self.stdout.write(
                        f"  {ip.date} | {ip.close_index} | {ip.volume_agg} | {ip.constituents_count} | {ip.method}"
                    )

                if (
                    DataIndustryPrice.objects.filter(industry=stock.industry_id).count()
                    > 5
                ):
                    self.stdout.write("  ...")
