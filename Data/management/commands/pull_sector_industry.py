...
class Command(BaseCommand):
    help = (
        "Pull sector/industry data from Yahoo Finance with database engine validation"
    )
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if not symbols:
                self.stdout.write(
                    self.style.ERROR(
                        "No symbols to process. Either provide --symbols or ensure Stock table has data."
                    )
                )
                return

            self.stdout.write(
                f"Processing {len(symbols)} symbols for sector/industry data"
            )
            self.stdout.write(
            ...
            self.stdout.write(
                f"Fetching data for {len(symbolsToFetch)} symbols that need updates"
            )
            ...
            # Upsert to database
            created, updated, skipped = yahoo_finance_service.upsertCompanyProfiles(
                profiles
            )
