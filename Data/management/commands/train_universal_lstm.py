...
from Data.models import Stock, DataSector
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=str,
            default='1.0',
            help='Model version for saving (default: 1.0)'
        )
        parser.add_argument(
            '--model-type',
            type=str,
        ...
        parser.add_argument(
            '--sector-balancing',
            action='store_true',
            help='Enable sector balancing during training'
        )
        parser.add_argument(
            '--cross-validation',
            action='store_true',
        ...
        parser.add_argument(
            ...
            type=str,
            default='all',
            help='Sectors to train on (default: all)'
        )
        parser.add_argument(
            '--min-stocks-per-sector',
            type=int,
            default=5,
            help='Minimum stocks per sector (default: 5)'
        )
        parser.add_argument(
            '--max-stocks-per-sector',
            type=int,
            default=15,
            help='Maximum stocks per sector (default: 15)'
        )
        ...
        parser.add_argument(
            '--force-refresh',
            action='store_true',
            help='Force refresh all data from Yahoo Finance even if it exists'
        )
        parser.add_argument(
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS('Universal LSTM training completed successfully!')
            )
    ...
    def _check_gpu_availability(self):
        ...
        try:
            if torch.cuda.is_available():
                ...
                self.stdout.write(
                    self.style.WARNING('GPU not available, falling back to CPU training')
                )
        except ImportError:
    ...
    def _prepare_training_data(self, config: UniversalLSTMTrainingConfig, options: Dict[str, Any]) -> None:
        ...
        self.stdout.write(f'   >>> Stocks: {sync_results["new_stocks"]} new, {sync_results["existing_stocks"]} existing')
        ...
        if balance_results['balanced']:
            ...
            self.stdout.write(f'   >>> Sector balance: [WARN] {len(balance_results["underrepresented_sectors"])} sectors need more stocks')
        ...
        if options.get('force_refresh'):
            ...
            if not stocks_needing_sync:
                self.stdout.write(self.style.SUCCESS('   >>> All stocks have sufficient data - skipping Yahoo Finance sync'))
                return
        ...
        for i in range(0, total_stocks, batch_size):
            ...
            if self.check_interrupt():
                self.save_progress('data_collection', {
    ...
    def _collect_training_data(self, config: UniversalLSTMTrainingConfig) -> Dict[str, Any]:
        ...
        for i, symbol in enumerate(all_training_stocks, 1):
            ...
            try:
                ...
                price_data = price_reader.get_stock_prices(
                    symbol=symbol,
        ...
        if successful_stocks < 50:  # Minimum threshold
    ...
    def _create_universal_model(
        ...
        model = create_universal_lstm_model(
            input_size=24,  # Universal features (excluding embeddings)
    ...
    def _execute_training_pipeline(
        ...
        stage1_results = self._train_stage(
            model, train_features, train_targets, train_sector_ids, train_industry_ids,
            val_features, val_targets, val_sector_ids, val_industry_ids,
            config.foundation_epochs, config.foundation_lr, config.foundation_batch_size,
            device, "Foundation", 1, 3, config
        )
        ...
        stage2_results = self._train_stage(
            model, train_features, train_targets, train_sector_ids, train_industry_ids,
            val_features, val_targets, val_sector_ids, val_industry_ids,
            config.specialization_epochs, config.specialization_lr, config.specialization_batch_size,
            device, "Specialization", 2, 3, config
        )
        ...
        stage3_results = self._train_stage(
            model, train_features, train_targets, train_sector_ids, train_industry_ids,
            val_features, val_targets, val_sector_ids, val_industry_ids,
            config.finetuning_epochs, config.finetuning_lr, config.finetuning_batch_size,
            device, "Fine-tuning", 3, 3, config
        )
    ...
    def _train_stage(
        self, model, train_features, train_targets, train_sector_ids, train_industry_ids,
        val_features, val_targets, val_sector_ids, val_industry_ids,
        epochs, learning_rate, batch_size, device, stage_name, stage_num=1, total_stages=3, config=None
    ):
        ...
        print(f"[CURRICULUM]   Weights: MSE={weights['mse']:.2f}, Dir={weights['direction']:.2f}, Div={weights['diversity']:.2f}, Sec={weights['sector']:.2f}")
        ...
        for epoch in range(epochs):
            ...
            if self.check_interrupt():
                self.save_progress(f'training_{stage_name}', {
            ...
            for batch_features, batch_targets, batch_sectors, batch_industries in train_loader:
                ...
                if scaler is not None:
                    ...
                    torch.nn.utils.clip_grad_norm_(model.parameters(), config.gradient_clipping if config else 1.0)  # Gradient clipping
                ...
                if train_batches % 25 == 0:
                    with torch.no_grad():
                        ...
                        loss_components = criterion.get_component_losses(
                            predictions, batch_targets, batch_sectors
                        )
                        ...
                        if train_batches % 100 == 0:  # Log every 100 batches
                            print(f"     [DIVERSITY] Batch {train_batches}: Var={prediction_variance:.6f}, "
            ...
            if epoch % 5 == 0 or epoch == epochs - 1:
                ...
                self.stdout.write(f'  {loss_indicator} Epoch {epoch+1:>3}/{epochs} [{progress_bar}] '
                ...
                if epoch % 25 == 0 and sector_total:
                    ...
                    for sector_id, total in sector_total.items():
                        if total > 0:
                            ...
                            self.stdout.write(f'       Sector {sector_id}: {sector_acc:.1f}% ({sector_correct[sector_id]}/{total})')
        ...
        return {
            ...
            'sector_performance': {
                ...
                for sector_id in sector_total if sector_total[sector_id] > 0
    ...
    def _save_universal_model(
        ...
        self.stdout.write(f'Saving fitted scalers: feature_scaler={type(scalers["feature_scaler"]).__name__}, target_scaler={type(scalers["target_scaler"]).__name__}')
        
        # Get sector mappings
        from Analytics.ml.sector_mappings import SECTOR_MAPPING, INDUSTRY_MAPPING
        sector_mappings = {
    ...
    def _display_training_banner(self, options: Dict[str, Any]):
        ...
        self.stdout.write(f'   [EPOCHS] Total Epochs: {options["stage1_epochs"] + options["stage2_epochs"] + options["stage3_epochs"]}')
    ...
    def _display_training_summary(
        ...
        self.stdout.write(f'   [STAGE 2] Validation Accuracy: {self.style.SUCCESS(f"{accuracy_pct:.1f}%")} [{progress_bar}]')
        self.stdout.write(f'   v Final Loss: {self.style.SUCCESS(f"{training_results["final_loss"]:.6f}")}')
        self.stdout.write('')
        ...
        for stage_name, metrics in stages:
            if metrics:
                ...
                self.stdout.write(f'   {status} {stage_name:<15} '
                                f'{stage_accuracy:>5.1f}% accuracy  '
