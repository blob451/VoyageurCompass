...
class UserRegistrationSerializer(serializers.ModelSerializer):
    ...
    secret_question = serializers.CharField(
        required=True, max_length=255, help_text="Security question for password recovery"
    )
    secret_answer = serializers.CharField(
        write_only=True, required=True, min_length=3, help_text="Answer to the security question"
    )
    ...
    def create(self, validated_data):
        ...
        security_profile = UserSecurityProfile(user=user, secret_question=secret_question)
...
class PasswordResetRequestSerializer(serializers.Serializer):
    ...
    secret_answer = serializers.CharField(required=True, write_only=True, help_text="Answer to your security question")
    ...
    def validate(self, attrs):
        ...
        if not security_profile.check_secret_answer(secret_answer):
            ...
            if security_profile.failed_reset_attempts >= 5:
                raise serializers.ValidationError("Too many failed attempts. Please contact admin for assistance.")
...
class PasswordResetSerializer(serializers.Serializer):
    ...
    new_password2 = serializers.CharField(
        required=True, write_only=True, style={"input_type": "password"}, label="Confirm New Password"
    )
...
class AdminPasswordResetRequestSerializer(serializers.ModelSerializer):
    ...
    username = serializers.CharField(required=True, help_text="Your username or email")
    ...
    def validate_username(self, value):
        ...
        existing = PasswordResetRequest.objects.filter(user=user, status="pending").exists()
