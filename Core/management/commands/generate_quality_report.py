...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--html", action="store_true", help="Generate HTML dashboard report"
        )
        parser.add_argument(
    ...
    def _collect_code_quality_metrics(self):
        ...
        try:
            ...
            result = subprocess.run(
                ["bandit", "-r", ".", "--configfile", "pyproject.toml", "-f", "json"],
                capture_output=True,
                text=True,
                cwd=settings.BASE_DIR,
            )
                ...
                with open(coverage_file, "r") as f:
                    ...
                    if match:
                        ...
                        coverage_data["backend_coverage"] = {
                            ...
                            "status": (
                                "passing" if percentage >= 38 else "below_threshold"
                            ),
                        }
        except Exception as e:
            if self.verbose:
    ...
    def _collect_performance_metrics(self):
        ...
        performance_data = {
            ...
            "database_performance": {"avg_query_time_ms": 0, "slow_queries_count": 0},
        }

        # Try to read Lighthouse results
        try:
            lighthouse_dir = (
                settings.BASE_DIR / "Design" / "frontend" / "lighthouse-results"
            )
        ...
        try:
            ...
            if baseline_file.exists():
                with open(baseline_file, "r") as f:
                    ...
                    performance_data["api_response_times"]["avg_response_time_ms"] = (
                        round(summary.get("overall_avg_response_time_ms", 0), 2)
                    )
        except Exception as e:
            if self.verbose:
    ...
    def _collect_dependency_metrics(self):
        ...
        return {
            "python_dependencies": {"total": 0, "outdated": 0, "vulnerable": 0},
            "javascript_dependencies": {"total": 0, "outdated": 0, "vulnerable": 0},
    ...
    def _collect_complexity_metrics(self):
        ...
        return {
            ...
            "lines_of_code": {"python": 0, "javascript": 0, "total": 0},
    ...
    def _calculate_quality_score(self, report):
        ...
        for tool, metrics in code_quality.items():
            if (
                metrics["status"] == "non_compliant"
                or metrics["status"] == "issues_found"
            ):
        ...
        lighthouse_avg = (
            sum(performance["lighthouse_scores"].values()) / 4
            if any(performance["lighthouse_scores"].values())
            else 0
        )
...
"""
    ...
    def _display_summary(self, report):
        ...
        self.stdout.write(
            style(
                f"Overall Quality Score: {score['overall_score']}/100 (Grade: {score['grade']})"
            )
        )
        ...
        for component, value in score["components"].items():
            self.stdout.write(
                f'  {component.replace("_", " ").title()}: {value:.1f}/100'
            )
