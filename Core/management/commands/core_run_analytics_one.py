...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            self.stdout.write(
                self.style.SUCCESS(f'Starting analytics for {symbol}')
            )
            ...
            # Success summary
            self.stdout.write(
                self.style.SUCCESS(f'[SUCCESS] Analytics completed successfully for {symbol}')
            )
    ...
    def report_data_coverage(self, symbol: str, coverage: Dict[str, Any]):
        ...
        try:
            ...
            if sector_info['has_data']:
                ...
                try:
                    sector_trading_days = stock.sector_id.prices.filter(
                        date__gte=sector_info['earliest_date'],
                        date__lte=sector_info['latest_date']
                    ).count() if stock.sector_id else 0
                except:
                ...
                if stock_info['has_data']:
                    ...
                    coverage_diff_pct = (coverage_diff / max(actual_trading_days, sector_trading_days) * 100) if max(actual_trading_days, sector_trading_days) > 0 else 0
            ...
            if industry_info['has_data']:
                ...
                try:
                    industry_trading_days = stock.industry_id.prices.filter(
                        date__gte=industry_info['earliest_date'],
                        date__lte=industry_info['latest_date']
                    ).count() if stock.industry_id else 0
                except:
                    industry_trading_days = 0
                    
                industry_coverage_pct = (industry_trading_days / industry_expected * 100) if industry_expected > 0 else 0
                ...
                if stock_info['has_data']:
                    ...
                    coverage_diff_pct = (coverage_diff / max(actual_trading_days, industry_trading_days) * 100) if max(actual_trading_days, industry_trading_days) > 0 else 0
                    self.stdout.write(f"  Difference from Stock: {coverage_diff} days ({coverage_diff_pct:.1f}%)")
                    logger.info(f"Industry vs Stock coverage difference: {coverage_diff} days ({coverage_diff_pct:.1f}%)")
            else:
    ...
    def perform_backfill(self, symbol: str, required_years: int, max_attempts: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if backfill_result['success']:
                self.stdout.write(
                    self.style.SUCCESS(f"[SUCCESS] Backfill successful after {backfill_result['attempts_used']} attempts")
                )
                ...
                self.stdout.write(
                    self.style.WARNING(f"[WARNING]  Backfill completed with issues after {backfill_result['attempts_used']} attempts")
                )
    ...
    def validate_final_coverage(self, coverage: Dict[str, Any], symbol: str):
        ...
        if not coverage['industry']['has_data']:
            self.stdout.write(
                self.style.WARNING(f"[WARNING]  No industry data available for {symbol}")
            )
