...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=str,
            required=True,
            help="Stock ticker symbol to analyze (required)",
        )
        parser.add_argument(
            "--horizon",
            type=str,
        ...
        parser.add_argument(
            ...
            type=int,
            default=3,
            help="Maximum backfill attempts (default: 3)",
        )
        parser.add_argument(
            "--required-years",
            type=int,
            default=2,
            help="Years of historical data required (default: 2)",
        )
        parser.add_argument(
            "--skip-backfill",
            action="store_true",
        ...
        try:
            ...
            if not skip_backfill:
                if self.needs_backfill(coverage_result):
                    ...
                    backfill_result = self.perform_backfill(
                        symbol, required_years, max_attempts
                    )
                    logger.info(f"Backfill result: {backfill_result}")
                else:
                    self.stdout.write(
                        "Step 3: Data coverage adequate, skipping backfill"
                    )
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"[SUCCESS] Analytics completed successfully for {symbol}"
                )
            )
    ...
    def ensure_postgresql_engine(self):
        ...
        if engine == "sqlite":
            ...
            self.stdout.write(
                self.style.WARNING(
                    f"[WARNING]  Unexpected database engine '{engine}'. Expected 'postgresql'."
                )
            )
    ...
    def report_data_coverage(self, symbol: str, coverage: Dict[str, Any]):
        ...
        try:
            ...
            if stock_info["has_data"]:
                days_available = (
                    stock_info["latest_date"] - stock_info["earliest_date"]
                ).days + 1
                expected_trading_days = int(
                    days_available * 0.714
                )  # ~252/365 ratio for display period
                ...
                try:
                    ...
                    actual_trading_days = stock.prices.filter(
                        date__gte=stock_info["earliest_date"],
                        date__lte=stock_info["latest_date"],
                    ).count()
                ...
                self.stdout.write(
                    f"  Date Range: {stock_info['earliest_date']} to {stock_info['latest_date']}"
                )
                ...
                logger.info(
                    f"Stock data coverage: {actual_trading_days}/{expected_trading_days} ({coverage_pct:.1f}%)"
                )
            else:
            ...
            if sector_info["has_data"]:
                sector_days = (
                    sector_info["latest_date"] - sector_info["earliest_date"]
                ).days + 1
                ...
                try:
                    sector_trading_days = (
                        stock.sector_id.prices.filter(
                            date__gte=sector_info["earliest_date"],
                            date__lte=sector_info["latest_date"],
                        ).count()
                        if stock.sector_id
                        else 0
                    )
                ...
                sector_coverage_pct = (
                    (sector_trading_days / sector_expected * 100)
                    if sector_expected > 0
                    else 0
                )

                self.stdout.write(f"\nSector Data:")
                self.stdout.write(
                    f"  Date Range: {sector_info['earliest_date']} to {sector_info['latest_date']}"
                )
                ...
                if stock_info["has_data"]:
                    ...
                    coverage_diff_pct = (
                        (
                            coverage_diff
                            / max(actual_trading_days, sector_trading_days)
                            * 100
                        )
                    ...
                    self.stdout.write(
                        f"  Difference from Stock: {coverage_diff} days ({coverage_diff_pct:.1f}%)"
                    )
                    logger.info(
                        f"Sector vs Stock coverage difference: {coverage_diff} days ({coverage_diff_pct:.1f}%)"
                    )
            else:
            ...
            if industry_info["has_data"]:
                industry_days = (
                    industry_info["latest_date"] - industry_info["earliest_date"]
                ).days + 1
                ...
                try:
                    industry_trading_days = (
                        stock.industry_id.prices.filter(
                            date__gte=industry_info["earliest_date"],
                            date__lte=industry_info["latest_date"],
                        ).count()
                ...
                industry_coverage_pct = (
                    (industry_trading_days / industry_expected * 100)
                    if industry_expected > 0
                    else 0
                )

                self.stdout.write(f"\nIndustry Data:")
                self.stdout.write(
                    f"  Date Range: {industry_info['earliest_date']} to {industry_info['latest_date']}"
                )
                ...
                if stock_info["has_data"]:
                    ...
                    coverage_diff_pct = (
                        (
                            coverage_diff
                            / max(actual_trading_days, industry_trading_days)
                            * 100
                        )
    ...
    def check_data_coverage(self, symbol: str, required_years: int) -> Dict[str, Any]:
        ...
        try:
            ...
            for data_type, info in coverage.items():
                logger.info(
                    f"  {data_type}: has_data={info['has_data']}, gaps={info['gap_count']}"
                )
    ...
    def needs_backfill(self, coverage: Dict[str, Any]) -> bool:
        ...
        stock_needs = (
            not coverage["stock"]["has_data"] or coverage["stock"]["gap_count"] > 50
        )
        sector_needs = (
            coverage["sector"]["gap_count"] > 50
            if coverage["sector"]["has_data"]
            else False
        )
        industry_needs = (
            coverage["industry"]["gap_count"] > 50
            if coverage["industry"]["has_data"]
            else False
        )
        ...
        logger.info(
            f"Backfill assessment: stock={stock_needs}, sector={sector_needs}, industry={industry_needs}"
        )
        return needs_backfill

    def perform_backfill(
        self, symbol: str, required_years: int, max_attempts: int
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if backfill_result["success"]:
                ...
                self.stdout.write(
                    f"  Stock records: {backfill_result['stock_backfilled']}"
                )
                self.stdout.write(
                    f"  Sector records: {backfill_result['sector_backfilled']}"
                )
                self.stdout.write(
                    f"  Industry records: {backfill_result['industry_backfilled']}"
                )
            else:
                self.stdout.write(
                    self.style.WARNING(
    ...
    def validate_final_coverage(self, coverage: Dict[str, Any], symbol: str):
        ...
        stock_adequate = (
            coverage["stock"]["has_data"] and coverage["stock"]["gap_count"] <= 800
        )
        ...
        if not coverage["sector"]["has_data"]:
            self.stdout.write(
                self.style.WARNING(f"[WARNING]  No sector data available for {symbol}")
            )

        if not coverage["industry"]["has_data"]:
            self.stdout.write(
                self.style.WARNING(
                    f"[WARNING]  No industry data available for {symbol}"
                )
            )
    ...
    def run_analytics(self, symbol: str, horizon: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for indicator, values in result["components"].items():
                logger.info(
                    f"  {indicator}: raw={values['raw']}, score={values['score']:.3f}"
                )
    ...
    def print_verification_results(self, symbol: str, analysis_result: Dict[str, Any]):
        ...
        try:
            ...
            self.stdout.write(
                f"Composite Raw Score: {analysis_result['composite_raw']:.6f}"
            )
            ...
            for indicator, weighted_score in analysis_result["weighted_scores"].items():
                ...
                self.stdout.write(
                    f"  {indicator}: {float(weighted_score):.6f} (score: {component['score']:.3f})"
                )

        except Exception as e:
            logger.error(f"Error printing verification results: {str(e)}")
            self.stdout.write(
                self.style.ERROR(f"Error generating verification output: {str(e)}")
            )
        ...
        try:
            ...
            if prices.exists():
                ...
                for price in prices[:5]:
                    self.stdout.write(
                        f"  {price.date}: ${price.close} (Vol: {price.volume:,})"
                    )
                ...
                if stock.sector_id:
                    ...
                    if sector_prices.exists():
                        ...
                        for sp in sector_prices[:5]:
                            self.stdout.write(
                                f"  {sp.date}: Index {sp.close_index} ({sp.constituents_count} stocks)"
                            )
                ...
                if stock.industry_id:
                    ...
                    if industry_prices.exists():
                        ...
                        for ip in industry_prices[:5]:
                            self.stdout.write(
                                f"  {ip.date}: Index {ip.close_index} ({ip.constituents_count} stocks)"
                            )
            else:
