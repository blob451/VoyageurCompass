...
from Data.models import Stock, StockPrice, Portfolio, PortfolioHolding, DataSector, DataIndustry, AnalyticsResults
...
class UserJourneyIntegrationTest(TransactionTestCase):
    ...
    def test_complete_user_analysis_workflow(self):
        ...
        print(f"  - Analytics processing: {explanation_time:.1f}s" if 'explanation_time' in locals() else "  - Analytics: processed")
...
class CrossModuleIntegrationTest(TestCase):
    ...
    def setUp(self):
        ...
        self.client = APIClient()
        self.user = User.objects.create_user(
            username='cross_module_user',
            email='crossmodule@test.com',
            password='cross_module_pass_123'
        )
        self.client.force_authenticate(user=self.user)
    ...
    def test_data_analytics_integration(self):
        ...
        sector = DataSector.objects.create(
            sectorKey='tech_cross',
            sectorName='Technology Cross',
            data_source='yahoo'
        )
        
        stock = Stock.objects.create(
            symbol='TEST_CROSS',
            short_name='Cross Module Test',
            sector_id=sector
        )
