...
class APIChainIntegrationTest(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            ...
            password='api_integration_pass_123'
        )
        
        # Create test data
        self.sector = DataSector.objects.create(
            sectorKey='tech_api',
            sectorName='Technology API',
            data_source='yahoo'
        )
        
        self.industry = DataIndustry.objects.create(
            industryKey='software_api',
            industryName='Software API',
            sector=self.sector,
            data_source='yahoo'
        )
    ...
    def test_portfolio_management_api_chain(self):
        ...
        holding_response = self.client.post(
            reverse('data:portfolio-add-holding', args=[portfolio_id]),
            holding_data,
            format='json'
        )
        ...
        performance_response = self.client.get(
            reverse('data:portfolio-performance', args=[portfolio_id])
        )
        ...
        update_response = self.client.post(
            reverse('data:portfolio-update-holding', args=[portfolio_id]),
            update_data,
            format='json'
        )
    ...
    def test_market_data_api_chain(self):
        ...
        sync_response = self.client.post(
            reverse('data:stock-sync', args=[self.stock.id]),
            {'period': '1d'},
            format='json'
        )
    ...
    def test_error_propagation_api_chain(self):
        ...
        try:
            invalid_portfolio_response = self.client.post(
                reverse('data:portfolio-add-holding', args=[99999]),
...
class RealTimeAPIIntegrationTest(TransactionTestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            ...
            email='realtime@api.com',
            password='realtime_pass_123'
        )
        self.client.force_authenticate(user=self.user)
    ...
    def test_real_time_stock_data_api(self):
        ...
        sector = DataSector.objects.create(
            sectorKey='tech_realtime',
            sectorName='Technology Realtime',
            data_source='yahoo'
        )
        
        stock = Stock.objects.create(
            symbol='AAPL',
            short_name='Apple Inc. Realtime',
            currency='USD',
            exchange='NASDAQ',
            sector_id=sector
        )
        ...
        sync_response = self.client.post(
            reverse('data:stock-sync', args=[stock.id]),
            {'period': '1d'},
            format='json'
        )
        ...
        for url_name, method, data, max_time in test_cases:
            try:
                ...
                self.assertLess(response_time, max_time, 
                    f"API {url_name} should respond within {max_time}s")
