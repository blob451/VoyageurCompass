...
class DatabaseTransactionIntegrationTest(TransactionTestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="db_integration_user", email="db@integration.com", password="db_integration_pass_123"
        )

        self.sector = DataSector.objects.create(sectorKey="tech_db", sectorName="Technology DB", data_source="yahoo")

        self.industry = DataIndustry.objects.create(
            industryKey="software_db", industryName="Software DB", sector=self.sector, data_source="yahoo"
        )
        ...
        try:
            with transaction.atomic():
                ...
                portfolio = Portfolio.objects.create(
                    user=self.user, name="Rollback Test Portfolio", initial_value=Decimal("1000.00")
                )
    ...
    def test_foreign_key_cascade_operations(self):
        ...
        # Create related data
        stock = Stock.objects.create(
            symbol="CASCADE_TEST", short_name="Cascade Test Corp", sector_id=self.sector, industry_id=self.industry
        )

        portfolio = Portfolio.objects.create(
            user=self.user, name="Cascade Test Portfolio", initial_value=Decimal("5000.00")
        )
        ...
        holding = PortfolioHolding.objects.create(
            portfolio=portfolio, stock=stock, quantity=Decimal("10"), average_price=Decimal("150.00")
        )
    ...
    def test_concurrent_database_operations(self):
        ...
        concurrent_prices = StockPrice.objects.filter(stock__symbol__startswith="CONCURRENT_").count()
    ...
    def test_database_constraint_validation(self):
        ...
        with self.assertRaises(Exception):
            Stock.objects.create(
                symbol="FK_TEST", short_name="Foreign Key Test", sector_id_id=99999  # Non-existent sector
            )
...
class DatabasePerformanceBenchmarkTest(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="perf_test_user", email="perf@test.com", password="perf_test_pass_123"
        )

        self.sector = DataSector.objects.create(
            sectorKey="tech_perf", sectorName="Technology Performance", data_source="yahoo"
        )

        self.industry = DataIndustry.objects.create(
            industryKey="software_perf", industryName="Software Performance", sector=self.sector, data_source="yahoo"
        )
    ...
    def test_complex_query_performance(self):
        ...
        # Create test data
        stock = Stock.objects.create(
            symbol="QUERY_TEST", short_name="Query Test Corp", sector_id=self.sector, industry_id=self.industry
        )
        ...
        aggregation_result = StockPrice.objects.filter(
            stock=stock, date__gte=date.today() - timedelta(days=30)
        ).aggregate(avg_price=Avg("close"), max_price=Max("high"), min_price=Min("low"), total_volume=Count("volume"))
        ...
        portfolio = Portfolio.objects.create(
            user=self.user, name="Query Performance Portfolio", initial_value=Decimal("10000.00")
        )

        PortfolioHolding.objects.create(
            portfolio=portfolio, stock=stock, quantity=Decimal("50"), average_price=Decimal("102.00")
        )

        # Complex join query
        join_result = (
            PortfolioHolding.objects.select_related("portfolio__user", "stock__sector_id", "stock__industry_id")
            .filter(portfolio__user=self.user, stock__sector_id=self.sector)
            .first()
        )
...
class DatabaseMigrationIntegrationTest(TestCase):
    ...
    def test_database_indexes_performance(self):
        ...
        stock = Stock.objects.create(
            ...
            sector_id=DataSector.objects.first()
            or DataSector.objects.create(sectorKey="test_sector", sectorName="Test Sector", data_source="yahoo"),
        )
        ...
        date_results = StockPrice.objects.filter(date__gte=date.today() - timedelta(days=30))
