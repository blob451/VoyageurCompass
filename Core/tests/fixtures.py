...
from Core.models import BlacklistedToken, UserSecurityProfile
...
class CoreTestDataFactory:
    ...
    def create_test_user(username="testuser", email="test@example.com", password="TestPassword123!"):
        ...
        user = User.objects.create_user(
            username=username, email=email, password=password, first_name="Test", last_name="User"
        )
    ...
    def blacklist_token(token, user=None):
        ...
        return BlacklistedToken.objects.create(
            token=token, user=user, expires_at=timezone.now() + timedelta(days=7), reason="logout"
        )
    ...
    def create_multiple_users(count=5):
        ...
        for i in range(count):
            user = CoreTestDataFactory.create_test_user(username=f"testuser{i}", email=f"test{i}@example.com")
            users.append(user)
        return users
    ...
    def cleanup_test_data():
        ...
        UserSecurityProfile.objects.filter(user__username__startswith="test").delete()

        # Clean up blacklisted tokens
        BlacklistedToken.objects.filter(blacklisted_at__gte=timezone.now() - timedelta(hours=1)).delete()
...
class TestEnvironmentManager:
    ...
    def setup_test_environment():
        ...
        test_dirs = ["Temp/test_media", "Temp/test_static", "Temp/test_logs", "Temp/test_uploads"]
