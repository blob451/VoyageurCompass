    ...
    @extend_schema(
        ...
        responses={
            201: {
                ...
                "example": {
                    "user": {"id": 1, "username": "john_doe", "email": "john@example.com"},
                    "tokens": {"access": "eyJ0eXAiOiJKV1QiLCJhbGc...", "refresh": "eyJ0eXAiOiJKV1QiLCJhbGc..."},
                },
            }
        },
    )
...
class UserProfileView(APIView):
    ...
    @extend_schema(
        summary="Get user profile", description="Current user profile retrieval", responses={200: UserProfileSerializer}
    )
...
@extend_schema(
    ...
    responses={200: {"description": "API is healthy", "example": {"status": "healthy", "version": "1.0.0"}}},
)
@api_view(["GET"])
@permission_classes([AllowAny])
def health_check(request):
    """Basic API health status endpoint."""
    return Response({"status": "healthy", "version": "1.0.0", "service": "VoyageurCompass API"})
...
@extend_schema(
    ...
    responses={
        200: {"description": "User statistics", "example": {"portfolios": 3, "total_holdings": 15, "analyses_today": 5}}
    },
)
@api_view(["GET"])
@permission_classes([IsAuthenticated])
    ...
    @extend_schema(
        ...
        responses={
            200: {
                ...
                "example": {"secret_question": "What is your mother's maiden name?"},
            }
        },
    )
    def get(self, request):
        ...
        try:
            security_profile = user.security_profile
            return Response({"secret_question": security_profile.secret_question})
        except Exception:
    ...
    @extend_schema(
        ...
        responses={
            201: {
                ...
                "example": {"message": "Password reset request submitted to admin", "request_id": 1},
            }
        },
    )
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            logger.warning(f"Password reset requested for user {reset_request.user.username}: {reset_request.reason}")
...
class LogoutView(APIView):
    ...
    @extend_schema(
        ...
        request={
            "application/json": {
                "type": "object",
                "properties": {"refresh_token": {"type": "string", "description": "Refresh token to blacklist"}},
            }
        },
        responses={
            200: {
                "description": "Logout successful",
                "example": {"message": "Logout successful", "tokens_blacklisted": 2},
            }
        },
    )
    def post(self, request):
        ...
        try:
            ...
            if auth_header and auth_header.startswith("Bearer "):
                ...
                if user and BlacklistedToken.blacklist_token(access_token, user, "logout"):
                    tokens_blacklisted += 1

            refresh_token = request.data.get("refresh_token")
            if refresh_token and user:
                if BlacklistedToken.blacklist_token(refresh_token, user, "logout"):
                    tokens_blacklisted += 1

            username = user.username if user else "unknown"
            logger.info(f"User logged out: {username}, tokens blacklisted: {tokens_blacklisted}")

            return Response({"message": "Logout successful", "tokens_blacklisted": tokens_blacklisted})
            ...
            return Response(
                {"message": "Logout completed (with errors)", "tokens_blacklisted": 0}, status=status.HTTP_200_OK
            )
        ...
        try:
            ...
            if auth_header and auth_header.startswith("Bearer "):
                ...
                try:
                    ...
                    token_expires_at = timezone.datetime.fromtimestamp(decoded["exp"], tz=timezone.utc).isoformat()
                except Exception:
                    pass
            ...
            return Response({"valid": False, "detail": "Token validation failed"}, status=status.HTTP_401_UNAUTHORIZED)
