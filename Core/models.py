...
class UserSecurityProfile(models.Model):
    ...
    secret_question = models.CharField(max_length=255, help_text="Security question for password recovery")

    secret_answer_hash = models.CharField(max_length=255, help_text="Hashed answer to the security question")

    reset_token = models.UUIDField(null=True, blank=True, help_text="Temporary token for password reset")
    ...
    last_reset_attempt = models.DateTimeField(null=True, blank=True, help_text="Last password reset attempt timestamp")
...
class BlacklistedToken(models.Model):
    ...
    def blacklist_token(cls, token, user, reason="logout"):
        ...
        try:
            ...
            cls.objects.create(token=token, user=user, expires_at=expires_at, reason=reason)
            return True
        except Exception:
            return False
