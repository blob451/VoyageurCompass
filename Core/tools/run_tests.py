...
class TestRunner:
    ...
    def run_backend_tests(self, test_type="all", coverage=False, verbose=True):
        ...
        if coverage:
            cmd.extend(["--cov=Data", "--cov=Analytics", "--cov=Core", "--cov-report=term-missing"])
    ...
    def run_all_tests(self, args):
        ...
        if args.backend or args.all:
            success = (
                self.run_backend_tests(test_type=args.test_type, coverage=args.coverage, verbose=args.verbose)
                and success
            )

        if args.frontend or args.all:
            success = self.run_frontend_tests(coverage=args.coverage, watch=args.watch) and success
...
def main():
    ...
    parser.add_argument(
        "--test-type",
        choices=["all", "unit", "integration", "api", "models", "fast"],
        default="all",
