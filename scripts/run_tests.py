#!/usr/bin/env python
"""
Comprehensive test runner for VoyageurCompass project.
...
class TestRunner:
    ...
    def run_backend_tests(self, test_type="all", coverage=True, verbose=True):
    ...
    def run_frontend_tests(self, coverage=True, watch=False):
    ...
    def generate_coverage_report(self):
        ...
        # Backend coverage is generated by pytest
        backend_coverage_file = self.project_root / "htmlcov" / "index.html"

        # Frontend coverage
        frontend_coverage_file = self.frontend_dir / "coverage" / "index.html"

        print(f"Backend coverage report: {backend_coverage_file}")
        print(f"Frontend coverage report: {frontend_coverage_file}")

        # Generate combined report summary
        self.print_coverage_summary()

    def print_coverage_summary(self):
        """Print coverage summary from XML files."""
        try:
            import xml.etree.ElementTree as ET

            # Backend coverage
            backend_xml = self.project_root / "coverage.xml"
            if backend_xml.exists():
                tree = ET.parse(backend_xml)
                root = tree.getroot()
                coverage_attr = root.attrib
                if "line-rate" in coverage_attr:
                    backend_coverage = float(coverage_attr["line-rate"]) * 100
                    print(f"📈 Backend Test Coverage: {backend_coverage:.1f}%")

            # Frontend coverage would need similar parsing
            frontend_json = self.frontend_dir / "coverage" / "coverage-summary.json"
            if frontend_json.exists():
                ...
                with open(frontend_json) as f:
                    ...
                    print(f"📈 Frontend Test Coverage: {lines}%")
    ...
    def run_all_tests(self, args):
        ...
        if args.frontend or args.all:
            success = (
                self.run_frontend_tests(coverage=args.coverage, watch=args.watch)
                and success
            )

        if args.coverage and not args.watch:
...
def main():
    ...
    parser.add_argument(
        "--watch", action="store_true", help="Run frontend tests in watch mode"
    )
    ...
    parser.add_argument(
        "--quiet", dest="verbose", action="store_false", help="Quiet output"
    )
