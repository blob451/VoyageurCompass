...
class BlueGreenDeployment:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(f"Health check failed: {target}/{component} - Status: {response.status_code}")
    ...
    def deploy(self, services: List[str] = None, skip_health_check: bool = False) -> bool:
        """
    ...
    def status(self) -> Dict:
        ...
        for target in ["blue", "green"]:
            for component in ["backend", "frontend"]:
                ...
                status["services"][key] = "healthy" if self.health_check(target, component) else "unhealthy"
...
def main():
    ...
    parser.add_argument(
        "--nginx-host",
        default="localhost",
        help="Nginx host (default: localhost)"
    )
    parser.add_argument(
        "--nginx-port",
        type=int,
