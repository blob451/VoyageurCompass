...
class EnvironmentValidator:
    ...
    REQUIRED_VARS = {
        ...
        'staging': [
            'SECRET_KEY', 'DEBUG', 'ALLOWED_HOSTS', 'SITE_URL',
            'DB_NAME', 'DB_USER', 'DB_PASSWORD', 'DB_HOST',
            'REDIS_HOST', 'CORS_ALLOWED_ORIGINS'
        ],
        'development': [
    ...
    SECURITY_VARS = {
        'SECRET_KEY': lambda v: len(v) >= 50 and v != 'django-insecure-dev-key-change-this-in-production',
    ...
    def validate_required(self, environment: str) -> bool:
        ...
        if missing:
            self.errors.append(f"Missing {len(missing)} required variables for {environment} environment")
            return False
    ...
    def validate_security(self, environment: str) -> bool:
        ...
        if environment == 'development':
            self.info.append("ℹ️ Skipping security validation for development environment")
            return True
        ...
        for var, validator in self.SECURITY_VARS.items():
            ...
            if not value:
                ...
                if environment == 'production':
                    ...
                    self.warnings.append(f"Security variable {var} may have insecure value")
